...
cts <- as.matrix(read.table("./text_files/readcounts_qa.txt", sep="\t", row.names =1, header = TRUE))
cts <- as.matrix(cts[,-c(1:5)])
## change ncol to match number of columns
cts_numeric <- matrix(as.numeric(cts), ncol = 19)
...
```{r titles}
suz12 <- rowMeans(cts_numeric[, 1:3], na.rm = TRUE)
WT <- rowMeans(cts_numeric[, 4:7], na.rm = TRUE)
WT_0hr <- rowMeans(cts_numeric[, 11:13], na.rm = TRUE)
WT_24hr <- rowMeans(cts_numeric[, 8:10], na.rm = TRUE)
qa_0hr <- rowMeans(cts_numeric[, 14:15], na.rm = TRUE)
qa_24hr <- cts_numeric[, 19]
...
suz12 <- results(dds2, alpha=alpha, contrast=c("condition", "suz12", "WT"))
WT_0hr <- results(dds2, alpha=alpha, contrast=c("condition", "WT_0hr", "WT"))
WT_24hr <- results(dds2, alpha=alpha, contrast=c("condition", "WT_24hr", "WT"))
qa_0hr <- results(dds2, alpha=alpha, contrast=c("condition", "qa_0hr", "WT"))
qa_24hr <- results(dds2, alpha=alpha, contrast=c("condition", "qa_24hr", "WT"))
...
track = combine %>% select("SequenceID", "FeatureStart", "FeatureEnd", "VALUE", "suz12_sig.csv", "WT_0hr_sig.csv", "WT_24hr_sig.csv", "qa_0hr_sig.csv", "qa_24hr_sig.csv")

...
countdataInteractors <- read.table("../text_files/readcounts_qa_paper_FINAL.txt",skip=1, header=TRUE,stringsAsFactors=FALSE, row.names=1, check.names=FALSE, sep="\t")
...
ectopicTPM <- subset(allDataTPM, rownames(allDataTPM)%in%sanity_genes[,10])
...
AVERAGE_ectopicTPM <- subset(Averaged_Orderd_KO_data, rownames(Averaged_Orderd_KO_data)%in%sanity_genes[,10])
...
ggsave(filename = "histone_chaperone_boxplot_PAPER_DELETE.pdf", plot = box, dpi = 600, height = 5.5, width = 9)
```
...
#extracting genes not re-repressed
# Simple (assumes numeric)
# n <- ncol(GenesWithChanges)
# keep <- GenesWithChanges[, n-1] > GenesWithChanges[, 1] &
#         GenesWithChanges[, n]   > GenesWithChanges[, 1]
# Genes_subset <- GenesWithChanges[keep, , drop = FALSE]

#####################################################################################################
###Reclustering heatmap to split into genes groups recovered vs unrecovered and replotting.


# --- Inputs from earlier steps ---
# GenesWithChanges: matrix/data.frame with rows = genes (rownames), cols = samples
# altorder: desired column order
# early, late, unrec: character vectors of gene_ids per group (already intersected with rownames)
...
ggsave(filename = "./qa-suz12_log2_prc2_genes_heatmap_recovery_clustered.pdf", plot = heat_save, dpi=600, height=4, width=3)
...
##########FIGURE 2###############
#genes that dont recover K27 and are upregulated
venn.diagram(
  x = list(genes_unrecovered$gene_id, rownames(qa_24hr_K27), rownames(WT_24hr_K27)),
  category.names = c("H3K27me3 Unrecovered", "qa_24hr UP ", "WT_24hr UP"),
...
##########FIGURE 3###############
#ecoptic K27 genes that are downregulated
#read in DOWN genes

venn.diagram(
  x = list(sanity_genes$V10, rownames(qa_24hr_DOWN)),
  category.names = c("H3K27me3 Ectopic", "qa_24hr DOWN "),
  filename = 'ectopic_DOWN_24hr_genes_TEST.png',
  col=c("#8da0cb", "#66c2a5"),
  fill = c(alpha("#8da0cb",0.25), alpha("#66c2a5",0.25)),
  output=TRUE)

#Make euler plot
ectopic_gain_fit = euler(c("A" = 1671, "B" = 149, "A&B" = 36))
ectopic_gain_eulerplot <- plot(ectopic_gain_fit)
ggsave(filename = "Ectopic_K27_genes_Downregulated.pdf", plot = ectopic_gain_eulerplot, width = 4, height = 4, dpi = 600)
...
qa_WT_24hr_DOWN_fit = euler(c("A" = 1101, "B" = 129, "A&B" = 606))
...
```{r,}
## ------------------------------------------------------------------
## 1) Get DESeq2-normalized counts from your existing dds2
##    (these reflect size-factor normalization on your +1 counts)
## ------------------------------------------------------------------
norm_counts <- counts(dds2, normalized = TRUE)
expr_mat    <- log2(norm_counts + 1)    # keeps your pseudocount logic for log-scale

## ------------------------------------------------------------------
## 2) Average replicates to 4 columns using coldata$condition
##    Adjust the level names here if your coldata uses underscores.
## ------------------------------------------------------------------
# Ensure these are the level names you want; edit if needed:
# levels(coldata$condition)
# e.g., "WT_0hr","WT_24hr","qa_0hr","qa_24hr"

group_means <- sapply(levels(coldata$condition), function(g) {
  rowMeans(expr_mat[, coldata$condition == g, drop = FALSE], na.rm = TRUE)
})

# Reorder / rename to match your figure labels
Averaged_DESeq <- cbind(
  WT_0hr        = group_means[,"WT_0hr"],
  WT_24hr       = group_means[,"WT_24hr"],
  `qa_0hr`  = if ("qa_0hr" %in% colnames(group_means)) group_means[,"qa-suz12_0hr"] else group_means[,"qa_suz12_0hr"],
  `qa_24hr` = if ("qa_24hr" %in% colnames(group_means)) group_means[,"qa-suz12_24hr"] else group_means[,"qa_suz12_24hr"]
)

## ------------------------------------------------------------------
## 3) Subset to your ectopic genes (same list you used before)
##    Here I assume 'sanity_genes' is already loaded and gene IDs are in column 10.
## ------------------------------------------------------------------
AVERAGE_ectopic_DESeq <- Averaged_DESeq[
  rownames(Averaged_DESeq) %in% sanity_genes[,10], , drop = FALSE
]

# Optional: keep rows with any signal
GenesWithChanges_ectopic <- AVERAGE_ectopic_DESeq[
  rowSums(AVERAGE_ectopic_DESeq) > 0, , drop = FALSE
]

# Important: drop rows with zero variance (prevents NA/NaN in clustering)
nz <- apply(GenesWithChanges_ectopic, 1, sd, na.rm = TRUE) > 0
GenesWithChanges_ectopic <- GenesWithChanges_ectopic[nz, , drop = FALSE]

## ------------------------------------------------------------------
## 4) Heatmap (row-scaled Z) in your preferred column order
## ------------------------------------------------------------------
altorder <- c("WT_0hr","WT_24hr","qa-suz12_0hr","qa-suz12_24hr")

heatmap_ectopic <- pheatmap(
  GenesWithChanges_ectopic[, rev(altorder)],   # use rev() if it matches your panel layout
...
heatmap_ectopic_plot <- heatmap_ectopic[[4]]
ggsave("./qa-suz12_ectopic_genes_heatmap_row_scale_DESeqNorm.pdf",
       plot = heatmap_ectopic_plot, dpi = 600, height = 4, width = 3)

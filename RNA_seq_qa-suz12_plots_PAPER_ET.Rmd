...
dds <- DESeqDataSetFromMatrix(countData = round(cts_numeric),
                              colData = coldata,
                              design = ~ condition)
...
qa_0hr_K27_UP = subset(qa_0hr_RNA, rownames(qa_0hr_K27) %in% rownames(qa_0hr_UP))
WT_24hr_K27_UP = subset(WT_24hr_RNA, rownames(WT_24hr_K27) %in% rownames(WT_24hr_UP))
suz12_K27_UP = subset(suz12RNA, rownames(suz12_K27) %in% rownames(suz12_UP))
...
qa_0hr_all_K27_DOWN = subset(qa_0hr_all_K27, rownames(qa_0hr_all_K27) %in% (qa_0hr_DOWN))
WT_24hr_all_K27_DOWN = subset(WT_24hr_all_K27, rownames(WT_24hr_all_K27) %in% (WT_24hr_DOWN))
suz12_all_K27_DOWN = subset(suz12_all_K27, rownames(suz12_all_K27) %in% (suz12_DOWN))
...
venn.diagram(
  ...
  category.names = c("H3K27me3 Unrecovered", "qa_24hr UP "),
  filename = 'unrecovered_upregulated_genes.png',
  col=c("#8da0cb", "#fc8d62"),
  fill = c(alpha("#8da0cb",0.25), alpha("#fc8d62",0.25)),
  output=FALSE)

qa_WT_24hr_DOWN_fit = euler(c("A" = 262, "B" = 26, "A&B" = 45))
...
venn.diagram(
  x = list(sanity_genes$V10, rownames(qa_24hr_all_K27_DOWN)),
...
#Make euler plot
ectopic_gain_fit = euler(c("A" = 41, "B" = 149, "A&B" = 36))
...
qa_WT_24hr_UP_fit = euler(c("A" = 1101, "B" = 129, "A&B" = 606))
...
qa_WT_24hr_DE_fit = euler(c("A" = 1538, "B" = 391, "A&B" = 1301))
...
## ========================== ECTOPIC GENE SET ==========================
# Subset by ectopic list (gene IDs are in column 10 of sanity_genes)
ectopic_ids <- intersect(rownames(Averaged_DESeq_log2), sanity_genes[,10])

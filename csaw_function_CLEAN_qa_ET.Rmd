...
out <- make_chip_rna_heatmap_simple(
  ...
  chip_cols_list = list(
    K27 = c("logFC_cac1","logFC_cac2","logFC_ash1"),
    K36 = c("logFC_cac1","logFC_cac2","logFC_set7")
  ),
  rna_list  = list(cac1 = cac1RNA, cac2 = cac2RNA, set7 = set7RNA, ash1 = ash1RNA),
  gene_col  = "gene_id",
  manual_clusters = list(
    K36_lost       = K36lost,
    K27_lost_only  = K27lost_only
...
#now subsetting genes
library(data.table)

# --- 1) Start from your data.frames ---
g <- as.data.table(regions$allgenes_promTSSext)   # genes (keep all original cols)
p <- as.data.table(ectopic_sig)                   # peaks

# --- 2) Add join columns required by foverlaps (start/end) WITHOUT clobbering originals
# If your V2/V3 are BED 0-based, half-open [start, end), see BED NOTE below.
g[, `:=`(start = V2, end = V3, row_id = .I)]      # row_id lets us restore original order
p[, `:=`(start, end)]

# Keep only needed join cols from peaks (chrom, start, end)
p <- p[, .(seqnames, start, end)]

# --- 3) Keys and overlap join (genes as x, peaks as y) ---
setkey(g, V1, start, end)
setkey(p, seqnames, start, end)

ov <- foverlaps(g, p, nomatch = 0L)  # returns genes+peaks columns for overlapping rows

# --- 4) Keep the exact, original genes rows that had ≥1 overlap ---
keep_ids <- unique(ov$row_id)
genes_overlapping <- g[keep_ids]               # preserves all original columns

# Optional: restore original row order and drop helper cols
setorder(genes_overlapping, row_id)
genes_overlapping[, `:=`(row_id = NULL, start = NULL, end = NULL)]

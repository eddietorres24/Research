...
ggsave("ectopic_peak_coverage_pie.pdf", p, width = 3, height = 3)
...
###Doing similar appraoch for ectopic genes in qa 24 hr time point
...
##################################################
###I want to know what genes are located within ectopic peaks

#First, determine ectopic peaks that are significantly enriched

###MIGHT BE GOOD TO DO TO SUPPORT PEAK CALLING< BUT SKIPPING FOR NOW

#ectopic_sig = subset(H3K27me3_Results_K27peaks_ectopic_vWT_0hr, H3K27me3_Results_K27peaks_ectopic_vWT_0hr$logFC_qa_24hr > 1 & H3K27me3_Results_K27peaks_ectopic_vWT_0hr$FDR_qa_24hr < 0.05 & H3K27me3_Results_K27peaks_ectopic_vInput$logFC_WT_24hr < 1 & H3K27me3_Results_K27peaks_ectopic_vInput$logFC_WT_0hr < 1 )

#subset genes
# --- 0) start from your original data.frames
genes_df <- regions$allgenes_promTSSext   # has V1,V2,V3,V6,gene_id,...
peaks_df <- H3K27me3_Results_K27peaks_ectopic_vInput                   # has seqnames,start,end,...

# --- 1) add a stable row id so we can return rows exactly as they were
genes_df$row_id <- seq_len(nrow(genes_df))

# --- 2) GRanges: genes are BED-like (usually 0-based, half-open)
...
# --- 3) Build peaks GRanges two ways (0-based and 1-based) and choose the one that overlaps most
peaks_gr_0 <- makeGRangesFromDataFrame(
  peaks_df,
...
peaks_gr_1 <- makeGRangesFromDataFrame(
  peaks_df,
  seqnames.field = "seqnames",
  start.field    = "start",
  end.field      = "end",
...
n0 <- length(unique(queryHits(findOverlaps(genes_gr, peaks_gr_0, ignore.strand = TRUE))))
n1 <- length(unique(queryHits(findOverlaps(genes_gr, peaks_gr_1, ignore.strand = TRUE))))
peaks_gr <- if (n1 >= n0) peaks_gr_1 else peaks_gr_0
message(sprintf("Picked peaks as %s-based (overlaps: %d vs %d).", if (identical(peaks_gr, peaks_gr_1)) "1" else "0", n1, n0))

# --- 4) ANY overlap, ignore strand; keep genes exactly as-is
hits <- findOverlaps(genes_gr, peaks_gr, ignore.strand = TRUE, minoverlap = 1, type = "any")
keep_ids <- mcols(genes_gr)$row_id[unique(queryHits(hits))]
genes_overlapping <- genes_df[sort(keep_ids), ]

# --- 5) drop helper column
genes_overlapping$row_id <- NULL

ectopic_genes = genes_overlapping
ectopic_genes_BED = subset(allgenes, allgenes$V10 %in% ectopic_genes$gene_id)
ectopic_code = subset(ectopic_genes_BED, ectopic_genes$gene_id %in% rownames(allDataTPM))

#Write bed file
write.table(ectopic_genes_BED, file = "ectopic_qa_genes.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
...
H3K27me3_GAIN_qa_24hr = subset(H3K27me3_Results_allgenes_promTSSext_vWT_0hr, H3K27me3_Results_allgenes_promTSSext_vWT_0hr$logFC_qa_24hr > 1 & H3K27me3_Results_allgenes_promTSSext_vWT_0hr$gene_id %in% all_qa_k27_genes$gene_id )

H3K27me3_LOSS_qa_24hr = subset(H3K27me3_Results_allgenes_promTSSext_vWT_0hr, H3K27me3_Results_allgenes_promTSSext_vWT_0hr$logFC_qa_24hr < -1 & H3K27me3_Results_allgenes_promTSSext_vWT_0hr$gene_id %in% all_qa_k27_genes$gene_id)

sanity = subset(H3K27me3_GAIN_qa_24hr, H3K27me3_GAIN_qa_24hr$gene_id %in% ectopic_genes$gene_id)
sanity_genes = subset(ectopic_genes_BED, ectopic_genes_BED$V10 %in% sanity$gene_id)

write.table(sanity_genes, file = "ectopic_qa_genes.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)

#Real Ectopic peaks (cut out < 0 log2FC over Input)
ectopic_peaks = subset(H3K27me3_Results_K27peaks_ectopic_vInput, H3K27me3_Results_K27peaks_ectopic_vInput$logFC_qa_24hr > 0.585)
ectopic_peaks = ectopic_peaks[,1:3]

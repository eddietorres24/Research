...
#reading in metdata
meta <- read.csv("../csaw_samples_files/csaw_qa-suz12_ET.csv")
metaQC <- read.csv("../csaw_samples_files/csaw_qa-suz12_QC_ET.csv")
...
regions <- list(K27peaks = read.table("../bed_files/WT_0_24_hr_qa_merge.bed"), K27peaks_ectopic = read.table("../bed_files/qa-suz12_no_WT_0_or_24.bed"), K27genes = read.table("../bed_files/H3K27me3_methylated_genes_FINAL.bed"), Allpeaks = read.table("../bed_files/qa-suz12_WT_K27_merged_FIXED_LGI.bed"), K27promTSSext = read.table("../bed_files/K27promTSSext.bed"), allgenes_promTSSext = read.table("../bed_files/allgenespromTSSext.bed")) 
...
```{r, Run QC function}
#This is where you change the parameters and run the function
run_diffbind_only(metaQC, out_dir = "qa_QC_DiffBindOnly")
run_chipqc_only(metaQC, out_dir = "qa_QC_ChIPQCOnly")
chipqc_metrics_to_excel(metaQC, out_dir = "qa_QC_ChIPQCOnly", xlsx_file = "ChIPQC_metrics.xlsx")
...
csaw_analysis(
  ...
  write_igv   = FALSE,
  mask_region = FALSE,
  out_dir     = "qa-suz12_analysis", #output dir, name whatever you want
...
```{r, Old manual clustering logic for Heatmaps, echo = FALSE}
...
make_csaw_heatmap_clusterby <- function(
  ...
  collapse_mutants = FALSE,     # if TRUE, use loss/keep/gain "any/all" logic like in chip_rna fn
  fold_cutoff      = 1,         # threshold; if NULL, derive from zero_white_band
  sort_reference   = c("WT"),
  sort_decreasing  = TRUE,

  # NEW: choose how rows are split (kept for back-compat; ignored if manual_clusters provided)
  split_mode       = c("auto", "by_no_signal_per_strain", "loss_gain_pair"),
  signal_threshold = 0,

  # NEW: manual clustering API (mirrors make_chip_rna_heatmap_simple)
  manual_clusters        = NULL,   # list of gene sets / data.frames / GRanges / character vectors
  manual_labels          = NULL,   # optional labels; defaults to names(manual_clusters) or "cluster_i"
  manual_keep_unassigned = TRUE,   # FALSE = drop anything not in any manual cluster
  manual_sort            = NULL,   # named list: label -> list(on="chip", strains=c(...), decreasing=TRUE/FALSE)

  # NEW: gene id column (if present). If absent, use rownames(df)
  gene_col         = "gene_id",

  # sorting knobs (chip-only here)
  sort_on          = c("chip"),
  sort_strains     = NULL,   # default = plot_cols
  # (no sort_chip here because we have one chip matrix in this function)
  ...
  # choose ID vector first (for manual cluster matching)
  gene_ids <- {
    if (!is.null(gene_col) && gene_col %in% names(df)) {
      as.character(df[[gene_col]])
    } else if (length(rownames(df)) && all(!is.na(rownames(df)))) {
      rownames(df)
    } else {
  ...
  rownames(M_all) <- gene_ids
  strains_all <- sub("^logFC_", "", colnames(M_all))

  # validate cluster_by & pick plotting columns
  if (!is.null(cluster_by) && !all(cluster_by %in% strains_all)) {
  ...
  # 'orig' matrix = un-capped values used for logic
  want_for_orig <- unique(c(plot_cols, cluster_by, sort_reference))
  have_for_orig <- intersect(want_for_orig, strains_all)
  ...
  # unify threshold behavior (like chip_rna fn): if fold_cutoff is NULL, derive from zero_white_band (abs)
  ...
  # ---------- helpers shared with manual + auto ----------
  .order_by <- function(idx, strains = sort_strains, dec = sort_decreasing) {
    cols <- if (is.null(strains)) colnames(orig) else intersect(strains, colnames(orig))
    if (!length(cols)) cols <- colnames(orig)
    sc <- rowMeans(orig[idx, cols, drop = FALSE], na.rm = TRUE)
  ...
  .genes_from_obj <- function(x) {
    ...
    if (is.data.frame(x)) {
      ...
      gcol <- cand[cand %in% cn][1]
      if (!length(gcol)) stop("manual_clusters data.frame has no gene id column.")
    ...
    if (inherits(x, "GRanges")) {
      ...
      cand <- c(gene_col,"gene_id","gene","Gene","GeneID","gene_name","Name","symbol","V10","V11")
      gcol <- cand[cand %in% cn][1]
      if (!length(gcol)) stop("manual_clusters GRanges has no gene id column.")
  ...
  if (!is.null(manual_clusters) && length(manual_clusters) > 0) {
    # ===== MANUAL PATH (mirrors make_chip_rna_heatmap_simple) =====
    labs <- if (!is.null(manual_labels)) {
      if (length(manual_labels) != length(manual_clusters))
        stop("manual_labels must match length(manual_clusters).")
      manual_labels
    } else {
      if (!is.null(names(manual_clusters)) && all(nzchar(names(manual_clusters)))) names(manual_clusters)
      else paste0("cluster_", seq_along(manual_clusters))
    }
    ...
    for (i in seq_along(manual_clusters)) {
      ...
      if (length(hit)) cl_vec[hit] <- labs[i]
    }

    if (isTRUE(manual_keep_unassigned)) {
      cl_vec[is.na(cl_vec)] <- "other"
      cluster_levels <- c(labs, "other")
    } else {
      keep <- !is.na(cl_vec)
      genes <- genes[keep]
      cl_vec <- cl_vec[keep]
      M     <- M[keep, , drop = FALSE]
      orig  <- orig[keep, , drop = FALSE]
      ord   <- seq_len(nrow(M))
      cluster_levels <- labs
    }
    cl_split <- factor(cl_vec, levels = cluster_levels, ordered = TRUE)

    # per-cluster ordering, allowing per-label overrides
    .order_by_manual <- function(idx, label){
      if (!is.null(manual_sort) && !is.null(manual_sort[[label]])) {
        spec <- manual_sort[[label]]
        st   <- spec$strains %||% sort_strains
        dec  <- if (!is.null(spec$decreasing)) isTRUE(spec$decreasing) else sort_decreasing
        return(.order_by(idx, strains = st, dec = dec))
      } else {
        return(.order_by(idx, strains = sort_strains, dec = sort_decreasing))
      }
    }
    ...
    for (cl in levels(cl_split)) {
      ...
      new_order <- c(new_order, idx[.order_by_manual(idx, cl)])
    }
    ord <- new_order
    row_split <- cl_split[ord]

  } else {
    # ===== AUTO PATH (same semantics as chip_rna fn) =====
    split_mode <- match.arg(split_mode)
    if (split_mode == "auto") {
      split_mode <- if (!is.null(cluster_by) && length(cluster_by) == 2L) "loss_gain_pair" else "by_no_signal_per_strain"
    }

    if (split_mode == "loss_gain_pair") {
      if (is.null(cluster_by) || length(cluster_by) != 2L)
        stop("loss_gain_pair requires exactly two 'cluster_by' strains.")
      a <- cluster_by[1]; b <- cluster_by[2]
      valsA <- orig[, a]; valsB <- orig[, b]
      sA <- ifelse(valsA <= -abs(cut), -1L, ifelse(valsA >= abs(cut), 1L, 0L))
      sB <- ifelse(valsB <= -abs(cut), -1L, ifelse(valsB >= abs(cut), 1L, 0L))
      ...
      lvl <- c(sprintf("loss in both (%s & %s)", a, b),
               ...
               "other")
      row_split <- factor(lab, levels = lvl, ordered = TRUE)

      # within-cluster ordering by mean across chosen strains (like chip_rna)
      new_order <- integer(0)
      for (cl in levels(row_split)) {
        idx <- which(row_split == cl); if (!length(idx)) next
        new_order <- c(new_order, idx[.order_by(idx, strains = sort_strains, dec = sort_decreasing)])
      }
      ord <- new_order
      row_split <- row_split[ord]

    } else if (split_mode == "by_no_signal_per_strain") {
      # same as before, but ordering matches the chip_rna style (.order_by)
      if (is.null(cluster_by) || length(cluster_by) == 0)
        stop("by_no_signal_per_strain requires non-empty cluster_by.")
      split_df <- lapply(cluster_by, function(s) {
        v <- orig[, s]
        no_sig <- !is.na(v) & v <= signal_threshold
        factor(ifelse(no_sig, paste0("no signal in ", s),
                             paste0("signal in ", s)),
               levels = c(paste0("no signal in ", s),
                          paste0("signal in ", s)), ordered = TRUE)
      })
      names(split_df) <- cluster_by
      split_df <- as.data.frame(split_df, check.names = FALSE)

      # lexicographic split, then per-terminal-bin order by mean signal across sort_strains
      code_mat <- sapply(split_df, as.integer)
      # primary ordering by split bins, second by .order_by score
      primary <- do.call(order, as.list(as.data.frame(code_mat)))
      # within each bin, apply .order_by
      idxs <- split(seq_len(nrow(M)), interaction(split_df, drop = TRUE))
      new_order <- unlist(lapply(idxs, function(ix) ix[.order_by(ix, strains = sort_strains, dec = sort_decreasing)]), use.names = FALSE)

      ord <- new_order
      row_split <- split_df[ord, , drop = FALSE]
    } else {
      stop("Unsupported split_mode: ", split_mode)
    }

    # If collapse_mutants = TRUE and length(cluster_by)>=1, emulate chip_rna's loss/keep/gain buckets
    if (isTRUE(collapse_mutants) && !is.null(cluster_by) && length(cluster_by) >= 1) {
      targets <- setdiff(cluster_by, "WT"); if (!length(targets)) targets <- cluster_by
      # compute state on uncapped 'orig' as in chip_rna
      state <- sapply(targets, function(s) {
        v <- orig[, s]
        ifelse(v <= -abs(cut), -1L, ifelse(v >= abs(cut), 1L, 0L))
      })
      if (is.null(dim(state))) state <- matrix(state, ncol = 1, dimnames = list(NULL, targets))
      loss_any <- apply(state == -1L, 1, any, na.rm = TRUE)
      keep_all <- apply(state ==  0L, 1, all, na.rm = TRUE)
      gain_any <- apply(state ==  1L, 1, any, na.rm = TRUE)
      ...
      cl_split <- factor(lab, levels = c(sprintf("loss in any (%s)", paste(targets, collapse=" | ")),
                                          sprintf("kept in all (%s)", paste(targets, collapse=" & ")),
                                          sprintf("gain in any (%s)", paste(targets, collapse=" | ")),
                                          "other"),
                         ordered = TRUE)

      # order within each collapsed bucket
      new_order <- integer(0)
      for (cl in levels(cl_split)) {
        idx <- which(cl_split == cl); if (!length(idx)) next
        new_order <- c(new_order, idx[.order_by(idx, strains = sort_strains, dec = sort_decreasing)])
  ...
  ht <- Heatmap(
    ...
    row_split = row_split,              # vector OR data.frame (nested split)
...
make_csaw_heatmap_clusterby(
  ...
  cluster_by = NULL,
  manual_clusters = K27_map,
  collapse_mutants = FALSE,      
  fold_cutoff = NULL,
  width_in  = 6.5,
  base_height_in = 1.5,
  per_row_height_in = 0.001,
  clip_negatives_to_zero = FALSE,
  color_limits     = c(0,5),
  zero_white_band  = 0,
  out_stem = "K27_auto_collapse"
)
...
WT_24hr= read.csv("../qa-suz12_RNA-seq_Analysis/csv_files/WT_24hr__vs__WT_0hr__alpha0.05__DE_absLFC_ge_1p0.csv", row.names = 1)
...
make_chip_rna_heatmap_simple <- function(
  ...
  .mk_col_fun <- function(lims, band, cols){
    ...
    if (!is.null(band)) {
      ...
      # No band at all → standard behavior
...
out <- make_chip_rna_heatmap_simple(
  chip_list = list(
    K27 = H3K27me3_Results_K27promTSSext_vInput
  ),
  chip_cols_list = list(
    K27 = c("logFC_WT_0hr","logFC_qa_4hr","logFC_qa_24hr")
  ),
  rna_list  = list(zerohr = qa_0hr_RNA, twofourhr = qa_24hr_RNA),
  gene_col  = "gene_id",
  manual_clusters = list(
    genes_early = genes_early,
    genes_late  = genes_late,
    genes_unrecovered_hmap = genes_unrecovered_hmap
  ),
  manual_labels = c("early recovered","late recovered","unrecovered"),
  manual_sort = list(
    "early recovered" = list(on="rna", strains=c("zerohr","twofourhr"), decreasing=FALSE),
    "late recovered"  = list(on="rna", strains=c("zerohr", "twofourhr"), decreasing=FALSE),
    "unrecovered" = list(on="rna", strains=c("zerohr", "twofourhr"), decreasing=FALSE)
  ),
  manual_keep_unassigned = FALSE,  
...
rna_cols <- c("cac-1","set-7","ash-1")  # (match column names in your z-score df)

# genes present in RNA z-score matrix
genes_keep <- rownames(rna_z)

# Filter each ChIP df to RNA genes
K27_f <- subset(H3K27me3_CS_Results_K27promTSS_vInput, gene_id %in% genes_keep)
K36_f <- subset(H3K36me3_Results_K27promTSS_vInput,   gene_id %in% genes_keep)

# (Recommended) Filter your manual cluster dfs to those same genes
K36lost_f      <- subset(K36lost, K36lost$gene_id %in% genes_keep)
K27lost_only_f <- subset(K27lost_only, K27lost_only$gene_id %in% genes_keep)
...
out <- make_chip_rna_heatmap_simple(
  ...
  chip_cols_list = list(
    K27 = c("logFC_WT","logFC_cac1","logFC_ash1"),
    K36 = c("logFC_WT","logFC_cac1","logFC_set7")
  ),
  rna_matrix = rna_z,
  rna_select = rna_cols,
  gene_col  = "gene_id",
  manual_clusters = list(
    K36_lost      = K36lost_f,
    K27_lost_only = K27lost_only_f
  ),
  manual_labels = c("K36 lost","K27 lost only"),
  manual_keep_unassigned = FALSE,   # <- drop anything not in those clusters
  manual_sort = list(
    "K36 lost"       = list(on="chip", chip="K27", strains=c("cac1"), decreasing=TRUE),
    "K27 lost only"  = list(on="chip", chip="K27", strains=c("cac1"), decreasing=TRUE)
  ),
  ...
  chip_zero_band = 0,
  rna_colors = rna_col_fun,  # your RdBu colorRamp2 mapper with white at 0
  row_gaps_mm = 0,
  out_dir = "csaw_heatmaps",
...
# (Recommended) Filter your manual cluster dfs to those same genes
clust1 <- subset(Kac_Gain_CAF, Kac_Gain_CAF$gene_id %in% genes_keep)
clust2 <- subset(Kac_Gain_set7_noCAF, Kac_Gain_set7_noCAF$gene_id %in% genes_keep)
clust3 <- subset(Kac_no_change, Kac_no_change$gene_id %in% genes_keep)
clust4 <- subset(Kac_unerich, Kac_unerich$gene_id %in% genes_keep)
...
out <- make_chip_rna_heatmap_simple(
  chip_list = list(
    ...
    K4 = H3K4me2_Results_K27genes_vInput,
    Kac = Kac_Results_K27promTSSext_vInput
  ),
  chip_cols_list = list(
    K27 = c("logFC_WT","logFC_cac1"),
    K4 =  c("logFC_WT","logFC_cac1","logFC_cac2","logFC_set7"),
    Kac = c("logFC_WT","logFC_cac1","logFC_cac2","logFC_set7")
  ),
  rna_matrix = rna_z,
  rna_select = rna_cols,
  gene_col  = "gene_id",
  ...
  manual_labels = c("Kac Gain CAF-1","Kac Gain set-7","Kac no change", "Kac unenriched"),
  manual_sort = list(
    "Kac Gain CAF-1" = list(on="chip", chip="Kac", strains=c("cac1","cac2"), decreasing=TRUE),
    "Kac Gain set-7" = list(on="chip", chip="Kac", strains=c("set7"), decreasing=TRUE),
    "Kac no change" = list(on="chip", chip="Kac", strains=c("WT","cac1","cac2","set7"), decreasing=TRUE),
    "Kac unenriched" =list(on="chip", chip="K4", strains=c("WT","cac1","cac2","set7"), decreasing=TRUE)
  ),
  manual_keep_unassigned = FALSE,  
  chip_colors_list = list(
    K27 = c("white","#00441B"),
...
#auto-clustering
out <- make_chip_rna_heatmap_simple(
  chip_df   = H3K27me3_CS_Heatmap_K27genes_proms_vInput,
  rna_list  = NULL,
  gene_col  = "gene_id",
  chip_cols = c("logFC_WT","logFC_cac1","logFC_cac2","logFC_cac3","logFC_set7"),
  cluster_by       = c("cac1","cac2","cac-3"),
  collapse_mutants = FALSE,
  ...
  row_gaps_mm = 0,
  out_dir = "csaw_heatmaps",
  file_prefix = "H3K27me3_CAF1_clusters",
  save_formats = c("pdf","png"),
  width_in = 8,
  dpi = 300
)
...
###RNA
qa_24hr_RNA_all = read.csv("../qa-suz12_RNA-seq_Analysis/csv_files/qa_24hr__vs__WT_0hr__alpha0.05__ALL_results_LFC.csv", row.names = 1)
WT_24hr_RNA_all = read.csv("../qa-suz12_RNA-seq_Analysis/csv_files/WT_24hr__vs__WT_0hr__alpha0.05__ALL_results_LFC.csv", row.names = 1)

#subsetting the results df to contain K27 marked genes (native and ectopic)

qa_all_scatter_vInput = subset(H3K27me3_Results_allgenes_promTSSext_vInput, H3K27me3_Results_allgenes_promTSSext_vInput$gene_id %in% all_qa_k27_genes$gene_id)

qa_all_scatter_vWT = subset(H3K27me3_Results_allgenes_promTSSext_vWT_0hr, H3K27me3_Results_allgenes_promTSSext_vWT_0hr$gene_id %in% all_qa_k27_genes$gene_id)
                   
qa_ectopic_scatter_vWT = subset(H3K27me3_Results_allgenes_promTSSext_vWT_0hr, H3K27me3_Results_allgenes_promTSSext_vWT_0hr$gene_id %in% ectopic_genes$gene_id)
            
#K27vInput vs RNA qa 24 hr
out <- chip_scatter_flex(
  mode        = "chip_vs_rna",
  chip_df_x   = qa_all_scatter_vInput,   
  strain_x    = "qa_24hr",
  rna_df      = qa_24hr_RNA_all,                          # RNA df
  up_thresh      =  1,
  down_thresh    = -1,
  chip_x_label = "K27",                      # label in axes/title
  out_dir     = "scatter_outputs",
  file_prefix = "K27vInput_vs_RNA_qa-24hr_K27_promTSSext",
  width_in    = 12, height_in = 9, dpi = 600
)
out$gg

#K27vWT vs RNA qa 24 hr
out <- chip_scatter_flex(
  mode        = "chip_vs_rna",
  chip_df_x   = qa_all_scatter_vWT,   
  strain_x    = "qa_24hr",
  rna_df      = qa_24hr_RNA_all,                          # RNA df
  up_thresh      =  1,
  down_thresh    = -1,
  chip_x_label = "K27",                      # label in axes/title
  out_dir     = "scatter_outputs",
  file_prefix = "K27vWT_vs_RNA_qa-24hr_K27_promTSSext",
  width_in    = 12, height_in = 9, dpi = 600
)
out$gg

#K27 ectopic genes vWT vs RNA qa 24 hr
out <- chip_scatter_flex(
  mode        = "chip_vs_rna",
  chip_df_x   = qa_ectopic_scatter_vWT,   
  strain_x    = "qa_24hr",
  rna_df      = qa_24hr_RNA_all,                          # RNA df
  up_thresh      =  1,
  down_thresh    = -1,
  chip_x_label = "K27",                      # label in axes/title
  out_dir     = "scatter_outputs",
  file_prefix = "K27_ectopic_vWT_vs_RNA_qa-24hr_K27_promTSSext",
  width_in    = 12, height_in = 9, dpi = 600
)
out$gg
...
# --- data ---
df <- data.frame(
  group = c("Early recovered", "Late recovered", "Unrecovered"),
  bp    = c(early_recovered_bp, late_recovered_bp, unrecovered_bp)
)
df$pct <- df$bp / sum(df$bp)
df$lbl <- sprintf("%s (%.1f%%)", df$group, 100*df$pct)

# --- custom light colors ---
base_colors <- c("#66c2a5", "#fc8d62", "#8da0cb")  # soft palette
light_colors <- alpha(base_colors, 0.6)            # lighter transparency

# --- plot ---
p <- ggplot(df, aes(x = "", y = bp, fill = group)) +
  ...
  ggtitle("Coverage by category")

p

ggsave("peak_coverage_pie.pdf", p, width = 3, height = 3)

###Also doing the same for regions in WT 24 hr (including ectopic peaks)
df2 <- data.frame(
  group = c("Ectopic", "Native"),
  bp    = c(bp_within, bp_outside)
)
df2$pct <- df2$bp / sum(df2$bp)
df2$lbl <- sprintf("%s (%.1f%%)", df2$group, 100*df2$pct)

# --- custom light colors ---
base_colors <- c("#66c2a5", "#fc8d62")  # soft palette
light_colors <- alpha(base_colors, 0.6)            # lighter transparency

# --- plot ---
p2 <- ggplot(df2, aes(x = "", y = bp, fill = group)) +
  geom_col(width = 1, color = "white") +
  coord_polar(theta = "y") +
  scale_fill_manual(values = light_colors) +
  geom_text(aes(label = lbl),
...
ggsave("ectopic_peak_coverage_pie.pdf", p2, width = 3, height = 3)

###For 4hr peaks, telomeric vs internal
df3 <- data.frame(
  group = c("Internal", "subtelomeric"),
  bp    = c(bp_4hr_internal, bp_4hr_subtelomeres)
)
df3$pct <- df3$bp / sum(df3$bp)
df3$lbl <- sprintf("%s (%.1f%%)", df3$group, 100*df3$pct)
...
# --- plot ---
p3 <- ggplot(df3, aes(x = "", y = bp, fill = group)) +
  geom_col(width = 1, color = "white") +
  coord_polar(theta = "y") +
  scale_fill_manual(values = light_colors) +
  geom_text(aes(label = lbl),
...
ggsave("4hr_peak_coverage_pie.pdf", p3, width = 3, height = 3)

###Ectopic peaks, subtelomeric vs. internal

df4 <- data.frame(
  group = c("Internal", "subtelomeric"),
  bp    = c(bp_inside, bp_outside)
)
df4$pct <- df4$bp / sum(df4$bp)
df4$lbl <- sprintf("%s (%.1f%%)", df4$group, 100*df4$pct)
...
# --- plot ---
p4 <- ggplot(df4, aes(x = "", y = bp, fill = group)) +
  geom_col(width = 1, color = "white") +
  coord_polar(theta = "y") +
  scale_fill_manual(values = light_colors) +
  geom_text(aes(label = lbl),
...
ggsave("ectopic_location_peak_coverage_pie.pdf", p4, width = 3, height = 3)


```

```{r, Processing Dataframes}
###K27 Peaks regained in timecourse
###df from other script. convert row names to columns
library(dplyr)
library(tidyr)

K27_FilteredHeatmap_df2 <- K27_FilteredHeatmap_df %>%
  tibble::rownames_to_column("rid") %>%
  tidyr::extract(
    rid,
  ...
  relocate(seqnames, start, end, .before = 1)

#determine groups
early_recover = subset(K27_FilteredHeatmap_df2, K27_FilteredHeatmap_df2$logFC_qa_4hrvInput_K27me3 > 1 | K27_FilteredHeatmap_df2$logFC_qa_0hrvInput_K27me3 > 1)

late_recover = K27_FilteredHeatmap_df2 %>%
  filter(if_any(all_of(c(5:7)), ~ coalesce(.x > 1, FALSE)))
                      
#get unqiue rows
keys <- c("seqnames","start","end")
key_str <- function(df) {
  do.call(paste, c(df[c("seqnames","start","end")], sep = "\u0001"))
}

early_key <- do.call(paste, c(early_recover[keys], sep = "\u0001"))
late_key  <- do.call(paste, c(late_recover[keys],  sep = "\u0001"))

late_only <- late_recover[!(late_key %in% early_key), ]

unrecovered <- subset(
  K27_FilteredHeatmap_df2,
  !(do.call(paste, c(K27_FilteredHeatmap_df2[keys], sep="\u0001")) %in%
      union(key_str(early_recover), key_str(late_recover)))
)

#Make dfs for bed

## --- helper: join a (seqnames,start,end) df to regions$K27peaks and keep BED coords ---
#NOTE: Need to change colnames accordingly
make_bed <- function(df, regions_k27) {
  # Coerce types to ensure exact key matches
  df <- transform(df,
    seqnames = as.character(seqnames),
    start    = as.integer(start),
    end      = as.integer(end)
  )
  regions_k27 <- transform(regions_k27,
    seqnames = as.character(seqnames),
    start    = as.integer(start),
    end      = as.integer(end)
  )
  # Inner join on the 3-key; keep K27peaks coordinates
  m <- merge(
  ...
  # Keep only BED cols, de-dup, and optionally sort
  bed <- unique(m[c("seqnames","start","end")])
  names(bed) <- c("seqnames","start","end")
  bed[order(bed$seqnames, bed$start, bed$end), ]
}

## --- build the three BED data.frames ---
# early_recover_BED   <- make_bed(early_recover,   regions$K27peaks)
# late_recover_BED    <- make_bed(late_only,       regions$K27peaks)    # your "late minus early" df
# unrecovered_BED  <- make_bed(unrecovered,  regions$K27peaks)    # in neither early nor late

early_recover_BED   <- make_bed(early_recover,   K27_FilteredHeatmap_df2)
late_recover_BED    <- make_bed(late_only,       K27_FilteredHeatmap_df2)    # your "late minus early" df
unrecovered_BED  <- make_bed(unrecovered,  K27_FilteredHeatmap_df2)    # in neither early nor late


#write beds
###NOTE: dont do this for windowed calls, do this for whole length peak calls (region mode) to plot in deeptools accordingly
write.table(early_recover_BED, file = "early_recovered_peaks.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)

write.table(late_recover_BED, file = "late_recovered_peaks.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)

write.table(unrecovered_BED, file = "unrecovered_peaks.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)

#calculating total length of peaks
# helper: sum of BED widths (half-open intervals: length = end - start)
total_bp <- function(bed) {
  start <- as.integer(bed$start)
  end   <- as.integer(bed$end)
  sum(pmax(end - start, 0L), na.rm = TRUE)
}

# compute totals
early_recovered_bp <- total_bp(early_recover_BED)
late_recovered_bp  <- total_bp(late_recover_BED)
unrecovered_bp     <- total_bp(unrecovered_BED)

###
#Doing similar appraoch for ectopic peaks in qa 24 hr time point
###

## --- 0) Prefilter by QA 24h > 1 ---
df_pref <- K27_FilteredHeatmap_df2 %>%
  filter(coalesce(logFC_qa_24hrvInput_K27me3, -Inf) > 1)

## --- 1) Ectopic peaks (BED: 0-based half-open → 1-based inclusive) ---
ect <- with(regions$K27peaks_ectopic,
            GRanges(seqnames = V1, ranges = IRanges(start = V2 + 1L, end = V3)))

## --- 2) Windows from the prefiltered df (already 1-based) ---
win <- with(df_pref,
            GRanges(seqnames = seqnames, ranges = IRanges(start = start, end = end)))

## --- 3) Overlap flag (any overlap = “within”) ---
within_flag <- overlapsAny(win, ect)

## ------- A) If you want TOTAL BP by class (uses start/end) -------
df_bp <- df_pref %>%
  mutate(within = within_flag,
         width  = pmax(as.integer(end) - as.integer(start), 0L))

bp_within  <- sum(df_bp$width[df_bp$within],  na.rm = TRUE)
bp_outside <- sum(df_bp$width[!df_bp$within], na.rm = TRUE)

bp_within; bp_outside


###
#Also testing locations of 4 hour recovery
###
#read in chrom length file
chromlength = read.table(file="../bed_files/neurospora_chrom_full.bed")

#filter subtelomeres
chromlength_internal <- transform(chromlength, V2 = V2 + 150000L, V3 = V3 - 150000L)

#subest early recovered peaks inside and outside of subtelomeres
gE <- with(early_recover, GRanges(seqnames, IRanges(start, end)))
gI <- with(chromlength_internal, GRanges(V1, IRanges(V2, V3)))

mask <- overlapsAny(gE, gI)  # TRUE = within internal regions

`4hr_internal`     <- early_recover[ mask, ]
`4hr_subtelomeric` <- early_recover[!mask, ]

total_bp <- function(df) sum(pmax(as.integer(df$end) - as.integer(df$start), 0L), na.rm = TRUE)

bp_4hr_internal     <- total_bp(`4hr_internal`)
bp_4hr_subtelomeres <- total_bp(`4hr_subtelomeric`)


###########
### Ectopic Peaks within subtelomeres

# Keep only windows that overlap ectopic peaks
ect_win <- win[within_flag]

# Convert chromlength_internal BED (currently 0-based half-open) → 1-based inclusive GRanges
# You created it by shrinking V2/V3 by 150kb each side; ensure valid intervals remain.
chromlength_internal <- chromlength_internal %>%
  rename(chr = V1, start0 = V2, end0 = V3)

internal_gr <- with(chromlength_internal, {
  # keep only rows where end0 > start0 after shrink
  keep <- which(end0 > start0)
  GRanges(
    seqnames = chr[keep],
    ranges   = IRanges(start = start0[keep] + 1L, end = end0[keep])
  )
})

# Reduce internal regions (merge any touching/overlapping intervals)
internal_gr <- GenomicRanges::reduce(internal_gr)

## ------------------------------
## 2) Partition ectopic windows by INTERNAL vs OUTSIDE
## ------------------------------

# Portion INSIDE internal genome (sum of intersected widths)
inside_parts  <- GenomicRanges::intersect(ect_win, internal_gr, ignore.strand = TRUE)
bp_inside     <- sum(width(inside_parts))

# Portion OUTSIDE internal genome (i.e., subtelomeric or gaps)
outside_parts <- GenomicRanges::setdiff(ect_win, internal_gr, ignore.strand = TRUE)
bp_outside    <- sum(width(outside_parts))

# (Optional) sanity check: total ectopic bp should equal inside + outside
bp_total_ect  <- sum(width(ect_win))

## ------------------------------
## 3) Counts of regions, too (optional)
## ------------------------------
n_inside  <- length(unique(findOverlaps(ect_win, internal_gr, ignore.strand = TRUE)@from))
n_outside <- length(ect_win) - n_inside

## ------------------------------
## 4) Tidy summary tibble
## ------------------------------
summary_bp <- tibble::tibble(
  class     = c("inside_internal", "outside_internal", "total_ectopic_windows"),
  n_regions = c(n_inside, n_outside, length(ect_win)),
  bp_sum    = c(bp_inside, bp_outside, bp_total_ect)
)

summary_bp

####################################
#Do for ectopic genes

library(dplyr)
library(tibble)
library(GenomicRanges)

## --- 1) Internal bounds as GRanges (1-based inclusive) ---

keep <- which(chromlength_internal$end0 > chromlength_internal$start0)
internal_gr <- with(chromlength_internal[keep, ],
                    GRanges(seqnames = chr, ranges = IRanges(start = start0 + 1L, end = end0)))
internal_gr <- GenomicRanges::reduce(internal_gr, ignore.strand = TRUE)

## --- 2) Ectopic genes as GRanges (GFF-like -> already 1-based inclusive) ---
# Use V10 (or V11) as a gene identifier if present
gene_id <- if ("V10" %in% names(ectopic_genes)) ectopic_genes$V10 else seq_len(nrow(ectopic_genes))

genes_gr <- with(ectopic_genes,
                 GRanges(seqnames = V1,
                         ranges   = IRanges(start = V2, end = V3),
                         strand   = V6,
                         gene_id  = gene_id))

## --- 3) Partition by internal bounds ---
# Flag genes overlapping internal regions (ignore strand)
inside_flag <- GenomicRanges::countOverlaps(genes_gr, internal_gr, ignore.strand = TRUE) > 0

genes_inside_gr <- genes_gr[ inside_flag]
genes_out_gr    <- genes_gr[!inside_flag]

# Region counts
n_inside  <- length(genes_inside_gr)
n_outside <- length(genes_out_gr)

## --- 4) Basepairs INSIDE vs OUTSIDE (handle partial overlaps) ---
# INSIDE bp: intersect genes with internal bounds
inside_clip <- GenomicRanges::intersect(genes_gr, internal_gr, ignore.strand = TRUE)
bp_inside   <- sum(width(inside_clip))

# OUTSIDE bp: gene parts not in internal bounds
outside_clip <- GenomicRanges::setdiff(genes_gr, internal_gr, ignore.strand = TRUE)
bp_outside   <- sum(width(outside_clip))

bp_total <- sum(width(genes_gr))  # equals bp_inside + bp_outside

summary_genes <- tibble(
  class     = c("genes_inside", "genes_outside", "total_genes",
                "bp_inside", "bp_outside", "bp_total"),
  value     = c(n_inside, n_outside, length(genes_gr),
                bp_inside, bp_outside, bp_total)
)
...
#################################################
### determining genes that experience recovery

#subsetting early and late recovered genes
genes_early = subset(H3K27me3_Results_K27promTSSext_vInput, (H3K27me3_Results_K27promTSSext_vInput$logFC_qa_4hr > 1 | H3K27me3_Results_K27promTSSext_vInput$logFC_qa_0hr > 1) & H3K27me3_Results_K27promTSSext_vInput$FDR_qa_4hr < 0.05)

genes_late = H3K27me3_Results_K27promTSSext_vInput %>%
  filter(if_any(all_of(c(18, 21, 24)), ~ coalesce(.x > 1, FALSE)))
genes_late = subset(genes_late, (genes_late$FDR_qa_24hr < 0.05 | genes_late$FDR_qa_12hr < 0.05 | genes_late$FDR_qa_8hr < 0.05) & ! genes_late$gene_id %in% genes_early$gene_id) 

genes_unrecovered_hmap = subset(H3K27me3_Results_K27promTSSext_vInput, ! H3K27me3_Results_K27promTSSext_vInput$gene_id %in% genes_early$gene_id & ! H3K27me3_Results_K27promTSSext_vInput$gene_id %in% genes_late$gene_id)

#Making files for bed output
genes_early_BED = subset(regions$K27genes, regions$K27genes$gene_id %in% genes_early$gene_id)
genes_late_BED = subset(regions$K27genes, regions$K27genes$gene_id %in% genes_late$gene_id)
genes_unrecovered = subset(regions$K27genes, ! regions$K27genes$gene_id %in% genes_early$gene_id & ! regions$K27genes$gene_id %in% genes_late$gene_id)

#
late_or_unrecovered = genes_union <- dplyr::bind_rows(genes_late_BED, genes_unrecovered) |> dplyr::distinct(V1, V2, V3, .keep_all = TRUE)

#write beds
write.table(genes_early_BED, file = "early_recovered_genes.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
write.table(genes_late_BED, file = "late_recovered_genes.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
write.table(genes_unrecovered, file = "unrecovered_genes.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)

##################################################
#Ectopic Peaks and Genes

#Any ectopic peaks in WT 24hr?
H3K27me3_peaks_GAIN_WT_24hr = subset(H3K27me3_Results_K27peaks_ectopic_vWT_0hr, H3K27me3_Results_K27peaks_ectopic_vWT_0hr$logFC_WT_24hr > 1)

#significant ectopic peaks in qa-suz12
H3K27me3_peaks_GAIN_qa_24hr = subset(H3K27me3_Results_K27peaks_ectopic_vWT_0hr, H3K27me3_Results_K27peaks_ectopic_vWT_0hr$logFC_qa_24hr > 1)

ectopic_peaks = regions$K27peaks_ectopic[overlapsAny(with(regions$K27peaks_ectopic, GRanges(V1, IRanges(V2, V3))), with(H3K27me3_peaks_GAIN_qa_24hr, GRanges(seqnames, IRanges(start, end)))), ]

write.table(ectopic_peaks, file = "ectopic_qa_peaks.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)

#I want to know ALL genes that have > H3K27me3 in qa 24 hr vs WT 0 hr
H3K27me3_GAIN_qa_24hr = subset(H3K27me3_Results_allgenes_promTSSext_vWT_0hr, H3K27me3_Results_allgenes_promTSSext_vWT_0hr$logFC_qa_24hr > 1 & H3K27me3_Results_allgenes_promTSSext_vWT_0hr$gene_id %in% all_qa_k27_genes$gene_id & H3K27me3_Results_allgenes_promTSSext_vWT_0hr$FDR_qa_24hr < 0.05 )

H3K27me3_LOSS_qa_24hr = subset(H3K27me3_Results_allgenes_promTSSext_vWT_0hr, H3K27me3_Results_allgenes_promTSSext_vWT_0hr$logFC_qa_24hr < -1 & H3K27me3_Results_allgenes_promTSSext_vWT_0hr$gene_id %in% all_qa_k27_genes$gene_id & H3K27me3_Results_allgenes_promTSSext_vWT_0hr$FDR_qa_24hr < 0.05)

#all genes w/ > K27me3
K27_gain_genes = subset(allgenes, allgenes$V10 %in% H3K27me3_GAIN_qa_24hr$gene_id)

# > K27me3, but only in ectopic peaks
ectopics = H3K27me3_GAIN_qa_24hr[overlapsAny(with(H3K27me3_GAIN_qa_24hr, GRanges(seqnames, IRanges(start, end))), with(H3K27me3_peaks_GAIN_qa_24hr, GRanges(seqnames, IRanges(start, end)))), ]

ectopics_genes = subset(allgenes, allgenes$V10 %in% ectopics$gene_id)

write.table(K27_gain_genes, file = "qa_K27_gain_genes.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
write.table(ectopic_genes, file = "ectopic_qa_genes.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)

#combining native and ectopic K27 genes

all_qa_k27_genes <- bind_rows(regions$K27genes, ectopic_genes) %>%
  # if either file uses V11 as the ID, fall back to it
  mutate(gene_id = if_else(is.na(gene_id) | gene_id == "", V11, gene_id)) %>%
  distinct(gene_id, .keep_all = TRUE) %>%
  arrange(V1, V2, V3)


#counting internal peaks in 4 hr time point
fourhr_peaks = read.table("../bed_files/qa-suz12_4hr_H3K27me3_Rep2_peaks_peaks.bed")

library(dplyr)
library(tibble)
library(GenomicRanges)
# library(IRanges)  # not strictly needed below

## --- 0) Inputs you already have ---
# fourhr_peaks <- read.table("../bed_files/qa-suz12_4hr_H3K27me3_Rep2_peaks_peaks.bed")
# chromlength, chromlength_internal from your earlier code (0-based BED)
# H3K27me3_Results_Allpeaks_vInput (data.frame with seqnames/start/end/metrics)

## --- 1) Build internal_gr (1-based inclusive) if not already present ---
if (!exists("internal_gr")) {
  chromlength_internal <- transform(chromlength, V2 = V2 + 150000L, V3 = V3 - 150000L)
  chromlength_internal <- chromlength_internal %>%
    rename(chr = V1, start0 = V2, end0 = V3)
  keep <- which(chromlength_internal$end0 > chromlength_internal$start0)
...
## --- 2) Filter results table for 4h thresholds ---
df4 <- H3K27me3_Results_Allpeaks_vInput %>%
  filter(coalesce(logFC_qa_4hr, -Inf) > 0.585,
         coalesce(FDR_qa_4hr, 1) < 0.05)

## --- 3) Convert both to GRanges (coordinate sanity: results = 1-based; BED = 0-based half-open) ---
res_gr <- with(df4, GRanges(seqnames = seqnames, ranges = IRanges(start = start, end = end)))

peaks_gr <- with(fourhr_peaks,
                 GRanges(seqnames = V1, ranges = IRanges(start = V2 + 1L, end = V3)))

## --- 4) All overlaps between filtered results and 4h peaks ---
hits <- GenomicRanges::findOverlaps(res_gr, peaks_gr, ignore.strand = TRUE)

# Make a tidy overlaps table (each row = one result–peak pair)
overlaps_tbl <- bind_cols(
  df4[queryHits(hits), ] %>% as_tibble(),
  fourhr_peaks[subjectHits(hits), ] %>%
    as_tibble() %>%
    rename(peak_chr = V1, peak_start0 = V2, peak_end0 = V3, peak_name = V4) %>%
    mutate(peak_start = peak_start0 + 1L, peak_end = peak_end0)
) %>%
  mutate(result_row = queryHits(hits),
         peak_row   = subjectHits(hits))

# If you want only unique peaks that overlap the filtered results:
peak_idx_sub <- unique(subjectHits(hits))
peaks_sub    <- peaks_gr[peak_idx_sub]

## --- 5) Inside vs outside basepairs for the overlapping peaks only (unique coverage) ---
# Union overlapping peaks to avoid double-counting bp
peaks_union <- GenomicRanges::reduce(peaks_sub, ignore.strand = TRUE)

inside_parts  <- GenomicRanges::intersect(peaks_union, internal_gr, ignore.strand = TRUE)
outside_parts <- GenomicRanges::setdiff(peaks_union,  internal_gr, ignore.strand = TRUE)

bp_inside  <- sum(width(inside_parts))
bp_outside <- sum(width(outside_parts))
bp_total   <- sum(width(peaks_union))  # should equal inside + outside

n_peaks_total  <- length(peaks_sub)
n_peaks_inside <- sum(GenomicRanges::countOverlaps(peaks_sub, internal_gr, ignore.strand = TRUE) > 0)
n_peaks_outside <- n_peaks_total - n_peaks_inside

summary_4h <- tibble(
  class     = c("inside_internal_bp", "outside_internal_bp", "total_unique_bp",
                "peaks_inside", "peaks_outside", "peaks_total"),
  value     = c(bp_inside, bp_outside, bp_total,
                n_peaks_inside, n_peaks_outside, n_peaks_total)
)

## --- 6) Outputs ---
overlaps_tbl    # all result–peak overlaps (joined info)
summary_4h      # bp + counts restricted to peaks overlapping significant 4h results

# --- 0) Choose output paths
out_dir <- "./"  # change if you want
bed_orig_path   <- file.path(out_dir, "peaks_insideInternal.bed")

# --- 1) Identify which of the (filtered-overlap) peaks are inside the internal bounds
inside_flag <- GenomicRanges::countOverlaps(peaks_sub, internal_gr, ignore.strand = TRUE) > 0
peaks_inside_gr <- peaks_sub[inside_flag]

# Map back to the original fourhr_peaks rows (so we can write original intervals if desired)
orig_rows_inside <- peak_idx_sub[inside_flag]

# --- 2A) BED of original peak intervals that overlap internal (0-based half-open)
# Handle presence/absence of a 4th "name" column gracefully
if (ncol(fourhr_peaks) >= 4) {
  bed_inside_orig <- tibble::tibble(
    V1 = fourhr_peaks$V1[orig_rows_inside],
    V2 = fourhr_peaks$V2[orig_rows_inside],           # already 0-based
    V3 = fourhr_peaks$V3[orig_rows_inside],           # half-open end
    V4 = fourhr_peaks$V4[orig_rows_inside]
  )
} else {
  bed_inside_orig <- tibble::tibble(
    V1 = fourhr_peaks$V1[orig_rows_inside],
    V2 = fourhr_peaks$V2[orig_rows_inside],
    V3 = fourhr_peaks$V3[orig_rows_inside]
  )
}

write.table(bed_inside_orig, bed_orig_path, sep = "\t",

...
csaw_prom <- regionCounts(bam.files = samples$bamReads, regions = prom_gr, param = param)
...
dge_prom  <- asDGEList(csaw_prom,  group = grouping)

#Estimate dispersions
disp_gene  <- estimateDisp(dge_gene,  design.mat)
disp_prom  <- estimateDisp(dge_prom,  design.mat)

#Fit GLMs
fit_gene  <- glmQLFit(disp_gene,  design.mat, robust = TRUE)
fit_prom  <- glmQLFit(disp_prom,  design.mat, robust = TRUE)
...
run_csaw_analysis <- function(
  ...
  for (strain in strains) {
    ...
    if (!(chip_col %in% colnames(design.mat)) || !(ref_col %in% colnames(design.mat))) {
      message("Skipping: ", strain, " — missing contrast columns: ", chip_col, " or ", ref_col)
      next
    }

...
```{r, read in files}
cac1DE <- read.csv("../CAF-1_RNA-seq_Analysis/csv_files/cac1_new.csv", sep = ",", header = TRUE, row.names = 1)
cac2DE <- read.csv("../CAF-1_RNA-seq_Analysis/csv_files/cac2.csv", sep = ",", header = TRUE, row.names = 1)
...
cac3DE$category <- case_when(
  cac3DE$log2FoldChange > -1 & cac3DE$log2FoldChange < 1 ~ "NS",
  rownames(cac3DE) %in% Prc2targets$V11 ~ "PRC2",
  cac3DE$log2FoldChange > 1 & cac3DE$padj < 0.05 ~ "Upregulated",
  cac3DE$log2FoldChange < -1 & cac3DE$padj < 0.05 ~ "Downregulated",
  TRUE ~ "NS"
)

# Reorder so PRC2 is plotted last (on top)
cac3DE <- cac3DE[order(cac3DE$category == "PRC2"), ]

# Plot
volcano_plot <- ggplot(cac3DE, aes(
  x = log2FoldChange,
  y = -log10(padj),
  color = category
)) +
  ...
  labs(
    x = expression(paste("log"[2], "FoldChange(cac3/WT)")),
...
ggsave("./cac3DE_volcano_plot_FINAL.pdf", plot=volcano_plot, width = 12, height = 9, unit="in",  dpi=400)
```

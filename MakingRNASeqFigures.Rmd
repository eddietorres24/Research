...
top_rows <- filter(tpm_vsd1, TPM >= quantiles_vsd1[2])
bottom_rows <- filter(tpm_vsd1, TPM <= quantiles_vsd1[1])
...
```{r}
gg <- ggplot(tpm_vsd1, aes(x = Sample, y = TPM, color = Color)) +
  geom_point(size = 3) +  scale_color_manual(values = c(Red = "red", Blue = "blue", Black = "black", SkyBlue = "skyblue")) +
...
```{r}
Rtt109DeSeq <- read.csv("~/Desktop/RochelleLabDesktopData/RNAseq/Rtt109/DeSeq2Results_032024/S534.csv", sep = ",", header = TRUE, row.names = 1)
head(Rtt109DeSeq)
```
...
```{r}
top_genes <- rownames(Rtt109DeSeq)[order(Rtt109DeSeq$pvalue)][1:15]
```
...
volcano_plot <- ggplot(cac1DE, aes(x = log2FoldChange, y = -log10(pvalue), color = ifelse(log2FoldChange < 0 & pvalue < 0.05, "Downregulated", ifelse(log2FoldChange > 0 & pvalue < 0.05, "Upregulated", "Not significant")))) +
  ...
  labs(x = expression(paste("log"[2], "FoldChange")), y = expression(paste("-log"[10], "(Adjusted p-value)")), title = "Rtt109 vs Wildtype") +
...
volcano_plot_with_labels <- volcano_plot +
  geom_text_repel(data = subset(Rtt109DeSeq, rownames(Rtt109DeSeq) %in% top_genes), aes(label = top_genes), size = 3, nudge_y = 0.2, force = 3)
...
```{r}
top_genes_wPRC2 <- c(rownames(Rtt109DeSeq)[order(Rtt109DeSeq$padj)][1:15], "NCU05460", "NCU05300", "set-7", "cac-3")

Rtt109DeSeq$label <- ifelse(rownames(Rtt109DeSeq) %in% top_genes_wPRC2, rownames(Rtt109DeSeq), "")

```
Define the volcano plot
```{r}
volcano_plot <- ggplot(Rtt109DeSeq, aes(x = log2FoldChange, y = -log10(pvalue), color = ifelse(log2FoldChange < 0 & pvalue < 0.05, "Downregulated", ifelse(log2FoldChange > 0 & pvalue < 0.05, "Upregulated", "Not significant")))) +
  geom_point(size = 2.5, alpha = 0.7) +
  scale_color_manual(values = c("Downregulated" = "blue", "Upregulated" = "red", "Not significant" = "black"),
                     labels = c("Downregulated", "Upregulated", "Not significant")) +
...
volcano_plot_with_labels <- volcano_plot +
  geom_point(data = subset(Rtt109DeSeq, rownames(Rtt109DeSeq) %in% K27SilentAcounts[,1]), aes(shape = "PRC2 Target Genes"), color = "#E7B800", size = 3) +
 geom_text_repel(data = subset(Rtt109DeSeq, !is.na(label)), aes(label = label), size = 3, nudge_y = 0.2, force = 3, color = "black", max.overlaps = Inf) 
  print(volcano_plot_with_labels)
```
...
```{r}
 Rtt109DeSeq$color <- ifelse(Rtt109DeSeq$pvalue >= 0.05, "black", 
                              ifelse(Rtt109DeSeq$log2FoldChange < 0, "Downregulated", "Upregulated"))
  Rtt109DeSeq$color[rownames(Rtt109DeSeq) %in% K27SilentAcounts[,1] & Rtt109DeSeq$pvalue < 0.05] <- "#E7B800"
```
Plot the volcano plot  


```{r}
  volcano_plot_with_labels3 <- ggplot(Rtt109DeSeq, aes(x = log2FoldChange, y = -log10(pvalue), color = color)) +
    geom_point(size = 2.5, alpha = 0.7) +
...
```{r}
PRC2TargetGenes_Rtt109DeSeq <- subset(Rtt109DeSeq, rownames(Rtt109DeSeq)%in%RevisedK27Genes[,1])
```

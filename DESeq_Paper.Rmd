...
cts2 <- as.matrix(read.table("./readcounts_no_isw_rtt109.txt", sep="\t", row.names =1, header = TRUE))
cts2 <- as.matrix(cts2[,-c(1:5)])
## change ncol to match number of columns
cts_numeric <- matrix(as.numeric(cts), ncol = 30)
...
dds$condition <- relevel(dds$condition, ref = "WTmycelia")
```
...
set3 <- results(dds2, alpha=alpha, contrast=c("condition", "set3", "WTmycelia"))
...
track = combine %>% select("SequenceID", "FeatureStart", "FeatureEnd", "VALUE", "cac1.csv", "cac2.csv", "cac3.csv", "set3.csv", "set7.csv")

write.table(track, file="cactrack_ash3.igv", sep="\t", row.names = FALSE, quote = FALSE)
```

...
csaw_analysis <- function(
  ...
  out_obj = NULL,
  reference_strain = "WT",
  fdr_cutoff = 0.05,       # used in window mode; also usable in region mode when mask_region=TRUE
  pvalue_cutoff = NULL,    # optional; same rules as FDR when provided
  mask_region = FALSE,     # region mode only: zero heatmap entries failing (FDR or P) thresholds
  ...
  require(edgeR); require(limma); require(csaw)
  require(GenomicRanges); require(S4Vectors)
  ...
  order_cols_wt_first <- function(nms, wt = "WT") {
    ...
    c(wt_hit, setdiff(nms, wt_hit))
  }

  # helper: significance mask builder given a results df column stem (FDR_, PValue_, logFC_)
  sig_mask <- function(df, stem, need_positive = FALSE) {
    fcol <- paste0("FDR_",    stem)
    pcol <- paste0("PValue_", stem)
    lcol <- paste0("logFC_",  stem)
    ok <- rep(TRUE, nrow(df))
    if (!is.null(fdr_cutoff)    && fcol %in% names(df)) ok <- ok & (df[[fcol]] <= fdr_cutoff)
    if (!is.null(pvalue_cutoff) && pcol %in% names(df)) ok <- ok & (df[[pcol]] <= pvalue_cutoff)
    if (need_positive && lcol %in% names(df)) ok <- ok & (df[[lcol]] > 0)
    ok
  }
  ...
  run_norm <- function(mod, fit, counts, region_tag, norm_type, gr_for_region = NULL) {
    ...
    if (norm_type != "input") strains <- setdiff(strains, reference_strain)
    ...
    for (st in strains) {
      ...
      if (norm_type == "input") {
        ...
        if (!(chip_col %in% colnames(design_mat)) || !(st_input_col %in% colnames(design_mat))) {
          message("    Skipping ", st, " (missing ", chip_col, " or ", st_input_col, ")"); next
        }
        ...
        need <- c(chip_col, st_input_col, ref_chip_col, ref_input_col)
        if (!all(need %in% colnames(design_mat))) {
          message("    Skipping ", st, " (missing: ", paste(setdiff(need, colnames(design_mat)), collapse=", "), ")"); next
        }
    ...
    for (st in names(res_list)) {
      ...
      lfc_col <- if ("rep.logFC" %in% mnames) "rep.logFC" else if ("logFC" %in% mnames) "logFC" else
    ...
    # heatmap matrix
    logfc_cols <- grep("^logFC_", names(final_df), value = TRUE)
    ...
    if (mode == "window") {
      if (norm_type == "input") {
        # keep rows where ANY strain is significant (FDR/P) AND has positive logFC vs input
        keep_any <- rep(FALSE, nrow(final_df))
        for (st in sub("^logFC_", "", logfc_cols)) {
          keep_any <- keep_any | sig_mask(final_df, stem = st, need_positive = TRUE)
        }
        heatmap_df <- heatmap_df[keep_any, , drop = FALSE]

        # save WT positive+significant mask (on the *unfiltered* full table alignment)
        if ("WT" %in% sub("^logFC_", "", logfc_cols)) {
          wt_possig_mask <- sig_mask(final_df, stem = "WT", need_positive = TRUE)
          names(wt_possig_mask) <- rownames(final_df)  # key by coord string
        }

        # also return which rows we kept so vWT can align if needed
        kept_rows <- rownames(heatmap_df)

      } else { # norm_type == "ref" (e.g., vWT)
        # keep rows if (A) any mutant is significant by FDR/P (no sign constraint)
        keep_any_mut <- rep(FALSE, nrow(final_df))
        for (st in sub("^logFC_", "", logfc_cols)) {
          keep_any_mut <- keep_any_mut | sig_mask(final_df, stem = st, need_positive = FALSE)
        }

        # OR (B) WT vInput was pos+sig at that locus (from the vInput run)
        # We'll read the mask later from the parent env if available.
        if (!exists(".wt_possig_lookup", inherits = FALSE)) {
          .wt_possig_lookup <- NULL
        }
        keep_wt_possig <- rep(FALSE, nrow(final_df))
        if (!is.null(.wt_possig_lookup)) {
          rn <- paste(final_df$seqnames, final_df$start, final_df$end, sep = ".")
          keep_wt_possig <- !is.na(.wt_possig_lookup[rn]) & .wt_possig_lookup[rn]
        }

        keep <- keep_any_mut | keep_wt_possig
        heatmap_df <- heatmap_df[keep, , drop = FALSE]
      }

    } else if (isTRUE(mask_region)) {
      # region mode: optional per-cell zeroing based on thresholds
      for (st in sub("^logFC_", "", logfc_cols)) {
        lcol <- paste0("logFC_", st)
        fcol <- paste0("FDR_", st)
        pcol <- paste0("PValue_", st)
        mask <- rep(FALSE, nrow(final_df))
        if (!is.null(fdr_cutoff)    && fcol %in% names(final_df)) mask <- mask | (final_df[[fcol]] > fdr_cutoff)
        if (!is.null(pvalue_cutoff) && pcol %in% names(final_df)) mask <- mask | (final_df[[pcol]] > pvalue_cutoff)
        if (any(mask)) heatmap_df[[lcol]][mask] <- 0
      }
    }
    # ----------------------------------------

    list(results = final_df, heatmap = heatmap_df, wt_possig_mask = wt_possig_mask)
  }
  ...
  if (mode == "region") {
    ...
    for (nm in names(out_obj$regions)) {
      ...
      for (mod in all_mods) {
        ...
        if (!is.null(res_vin)) {
          res_df <- res_vin$results; hm_df <- res_vin$heatmap
          ...
          if (write_igv) {
            lfc_cols <- grep("^logFC_", names(res_df), value = TRUE)
            igv_out  <- res_df[, c("seqnames","start","end","name", lfc_cols), drop = FALSE]
            write.table(igv_out, file.path(out_dir, paste0(mod, "_logFC_", nm, "_vInput.igv")),
        ...
        if (!is.null(res_vref)) {
          res_df <- res_vref$results; hm_df <- res_vref$heatmap
          ...
          if (write_igv) {
            lfc_cols <- grep("^logFC_", names(res_df), value = TRUE)
            igv_out  <- res_df[, c("seqnames","start","end","name", lfc_cols), drop = FALSE]
            write.table(igv_out, file.path(out_dir, paste0(mod, "_logFC_", nm, "_", norm_tag, ".igv")),
                        sep = "\t", quote = FALSE, row.names = FALSE)
          }
    ...
    if (!is.null(out_obj$genomewide)) {
      ...
      for (mod in all_mods) {
        ...
        # vInput first, so we can collect WT pos+sig mask for vWT pass
        res_vin <- run_norm(mod, fit, counts, region_tag = nm, norm_type = "input", gr_for_region = NULL)
        wt_lookup <- NULL
        if (!is.null(res_vin$wt_possig_mask)) {
          wt_lookup <- res_vin$wt_possig_mask
        }
        ...
        # Make WT vInput mask visible to the next call (vWT)
        .wt_possig_lookup <<- wt_lookup

        # vReference (e.g., vWT)
        res_vref <- run_norm(mod, fit, counts, region_tag = nm, norm_type = "ref", gr_for_region = NULL)
        .wt_possig_lookup <<- NULL  # clear after use
    ...
    if (!is.null(out_obj$regions) && length(out_obj$regions)) {
      for (nm in names(out_obj$regions)) {
        ...
        for (mod in all_mods) {
          ...
          res_vin <- run_norm(mod, fit, counts, region_tag = nm, norm_type = "input", gr_for_region = NULL)
          wt_lookup <- if (!is.null(res_vin$wt_possig_mask)) res_vin$wt_possig_mask else NULL

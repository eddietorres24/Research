...
# ============================================================
# SCRIPT: ChIPQC-only (per modification)
#   - No DiffBind here
#   - No tryCatch, no slow pre-count checks
#   - Drops samples with 0 reads-in-peaks before making report
#   - Forces PeakCaller="bed"
#   - Writes metrics CSVs and one report per Antibody
#   - Optional 'mods' to run only a subset (e.g., mods="H3K4me2")
# ============================================================
suppressPackageStartupMessages({
  library(ChIPQC)
...
# ---------- MAIN ----------
# metadata: data.frame or path to CSV with at least SampleID, Antibody, bamReads, Peaks
# out_dir : output directory
# mods    : NULL (all) or character vector of Antibody values to run (e.g., "H3K4me2")
run_chipqc_only <- function(metadata, out_dir = "QC_ChIPQCOnly", mods = NULL) {
  .safe_dir_qc(out_dir)
  ...
  # Basic normalize; exclude anything labelled like 'input'
  meta <- meta %>%
    mutate(
      SampleID  = as.character(SampleID),
      Antibody  = as.character(Antibody),
  ...
  if (!is.null(mods)) {
    meta <- meta %>% filter(Antibody %in% mods)
  }

  message("Total rows: ", nrow(meta))
  if (nrow(meta) == 0) stop("No rows to run in ChIPQC after filtering.")

  # Per-modification run
  message("\n[ChIPQC] Generating reports per modification…")
  for (mod in unique(meta$Antibody)) {
    ...
    # Minimal sheet; Tissue/Condition are optional, fill with simple values if absent
    chip_sheet <- mod_df %>%
      mutate(
        ...
        PeakCaller = "bed" # enforce BED to avoid score/width requirements
      ) %>%
      select(SampleID, Tissue, Factor, Condition, Replicate, bamReads, Peaks, PeakCaller)

    # Build once to get metrics
    chipObj_all <- ChIPQC(as.data.frame(chip_sheet))

    # Grab metrics; drop any 0 reads-in-peaks
    mets_all <- QCmetrics(chipObj_all)
    mets_csv <- file.path(mod_out, paste0("ChIPQC_metrics_all_", mod, ".csv"))
    .safe_dir_qc(mod_out)
    write.csv(mets_all, mets_csv, row.names = FALSE)

    keep_idx <- which(is.finite(mets_all$ReadsInPeaks) & mets_all$ReadsInPeaks > 0)
    if (length(keep_idx) == 0) {
      message("    No samples with reads-in-peaks > 0 for ", mod, " — skipping report.")
      next
    }
    # Optional: write filtered metrics
    mets_kept_csv <- file.path(mod_out, paste0("ChIPQC_metrics_kept_", mod, ".csv"))
    write.csv(mets_all[keep_idx, , drop=FALSE], mets_kept_csv, row.names = FALSE)

    # Subset object and report
    chipObj <- chipObj_all[keep_idx]
    ChIPQCreport(
      chipObj,
...
run_chipqc_only(dplyr::filter(metaQC, Antibody=="H3K4me2"),
                out_dir = "QC_ChIPQCOnly_fix")

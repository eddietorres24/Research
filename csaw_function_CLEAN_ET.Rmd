...
make_chip_rna_heatmap_simple <- function(
  chip_df,
  rna_list,                                 # named list of RNA dfs; rownames are gene IDs
  clusters_df = NULL,                       # optional; cols: gene, cluster
  gene_col    = "V10",
  chip_cols   = NULL,                       # NULL -> auto: logFC_<strain> for strains in rna_list
  rna_colname = "log2FoldChange",
  cluster_levels = NULL,                    # optional; if NULL and clusters_df provided, use its levels/order
  sort_strains   = NULL,                    # e.g. c("cac1","cac2"); if NULL, no sorting (or all RNA cols if clusters present)
  chip_limits    = c(-2, 2),
  rna_limits     = c(-10, 10),
  chip_zero_band = 0.5,
  rna_zero_band  = 2,
  ...
  strains <- names(rna_list)
  stopifnot(all(nzchar(strains)))

  # ---- HARD INTERSECTION: chip ∩ all RNA (∩ clusters if provided) ----
  ...
  if (!is.null(clusters_df)) {
    stopifnot(all(c("gene","cluster") %in% names(clusters_df)))
    common <- intersect(common, as.character(clusters_df$gene))
  }
  chip_df <- chip_df[match(common, genes_chip), , drop = FALSE]
  rna_list <- lapply(rna_list, function(df) df[common, , drop = FALSE])
  if (!is.null(clusters_df)) clusters_df <- clusters_df[clusters_df$gene %in% common, , drop = FALSE]
  genes <- common

  # ---- build matrices ----
  if (is.null(chip_cols)) {
    guess <- paste0("logFC_", strains)
  ...
  rnaMat <- do.call(cbind, lapply(strains, function(s) pull_rna(rna_list[[s]], genes, rna_colname)))
  colnames(rnaMat) <- paste0("RNA_", strains)
  ...
  if (!is.null(clusters_df)) {
    if (is.null(cluster_levels)) {
      # keep the order appearing in clusters_df if levels already set; else unique order
      if (is.factor(clusters_df$cluster)) {
        cluster_levels <- levels(clusters_df$cluster)
      } else {
        cluster_levels <- unique(as.character(clusters_df$cluster))
      }
    }
    clusters_df$cluster <- factor(as.character(clusters_df$cluster), levels = cluster_levels)
    cl_map <- setNames(as.character(clusters_df$cluster), clusters_df$gene)
    cl_vec <- cl_map[genes]
    cl_split <- factor(cl_vec, levels = cluster_levels)

    # sort within each cluster by mean over sort_strains (or all RNA cols if not provided)
    sort_cols <- if (is.null(sort_strains)) colnames(rnaMat) else paste0("RNA_", intersect(sort_strains, strains))
    score <- rowMeans(rnaMat[, sort_cols, drop = FALSE], na.rm = TRUE)
    idx <- unlist(lapply(levels(cl_split), function(cl){
      w <- which(cl_split == cl)
      if (!length(w)) integer(0) else w[order(score[w], decreasing = TRUE, na.last = TRUE)]
    }), use.names = FALSE)
    chipMat  <- chipMat[idx, , drop = FALSE]
    rnaMat   <- rnaMat [idx, , drop = FALSE]
    cl_split <- cl_split[idx]
  } else {
    # no clusters: optional global sort by mean of sort_strains
    if (!is.null(sort_strains)) {
      sort_cols <- paste0("RNA_", intersect(sort_strains, strains))
  ...
  # row gaps only when clusters are used
  row_gaps <- if (!is.null(cl_split)) {
    k <- sum(table(cl_split) > 0)
    if (k > 1 && row_gaps_mm > 0) unit(rep(row_gaps_mm, k - 1), "mm") else unit(0, "mm")
  } else unit(0, "mm")

  ht_chip <- Heatmap(
    chipMat, name = "ChIP log2FC", col = col_chip,
    cluster_rows = FALSE, cluster_columns = FALSE, show_row_names = FALSE,
    row_split = cl_split, row_gap = row_gaps
  )
  ht_rna <- Heatmap(
  ...
  invisible(list(chip_matrix = chipMat, rna_matrix = rnaMat, clusters = cl_split, genes_plotted = rownames(chipMat)))
}

# ---------------------- Example usage ----------------------
genes <- H3K36me3_Results_K27promTSS_vWT$V10  # or the gene list you want

rna_df <- data.frame(
  RNA_cac1 = cac1RNA[genes, "log2FoldChange"],
  RNA_cac2 = cac2RNA[genes, "log2FoldChange"],
  row.names = genes
)
...
out <- make_chip_rna_heatmap_simple(
  ...
  row_gaps_mm = 0,
  out_dir = "csaw_heatmaps",
  file_prefix = "H3K36me3_vs_RNA_clustered",
  save_formats = c("pdf","png"),
  width_in = 8,
  dpi = 300,
  cluster_levels = ord  
...
# 1) Get the plotted RNA matrix (in order)
rna_mat <- out$rna_matrix

# 2) Turn it into a data frame with gene + cluster + row index
rna_plotted <- cbind(
  gene    = rownames(rna_mat),
  as.data.frame(rna_mat),
  cluster = as.character(out$clusters)
)
rna_plotted$order_idx <- seq_len(nrow(rna_plotted))
rna_plotted <- rna_plotted[, c("order_idx","gene","cluster", setdiff(names(rna_plotted), c("order_idx","gene","cluster")))]


#RNA clustering
# Align by shared genes (rownames assumed to be gene IDs)
common <- Reduce(intersect, list(rownames(cac1RNA), rownames(cac2RNA), rownames(set7RNA)))
c1  <- cac1RNA[common, , drop = FALSE]
c2  <- cac2RNA[common, , drop = FALSE]
s7  <- set7RNA[common, , drop = FALSE]

up_cac1 <- !is.na(c1$log2FoldChange) & c1$log2FoldChange > 2
up_cac2 <- !is.na(c2$log2FoldChange) & c2$log2FoldChange > 2
up_set7 <- !is.na(s7$log2FoldChange) & s7$log2FoldChange > 2

cluster <- ifelse(up_cac1 & up_cac2 & up_set7, "cluster1_all_up",
           ifelse(up_cac1 | up_cac2, "cluster2_cac1_cac2_up",
           ifelse(!(up_cac1 | up_cac2) & up_set7, "cluster3_set7_up_only",
                  "cluster4_other")))

# Result table (handy for joins/plots)
clusters_df <- data.frame(
  gene = common,
  cluster = cluster,
  lfc_cac1 = c1$log2FoldChange,
  lfc_cac2 = c2$log2FoldChange,
  lfc_set7 = s7$log2FoldChange,
  row.names = NULL
)

clusters_df <- subset(clusters_df, clusters_df$gene %in% H3K27me3_TSS_Promoters$V10)

# order  (FIX the name to _cac1_cac2_)
ord <- c("cluster1_all_up", "cluster2_cac1_cac2_up", "cluster3_set7_up_only", "cluster4_other")

# set factor levels to ord (NOT to `cluster`)
clusters_df$cluster <- factor(clusters_df$cluster, levels = ord)
...
#you have to make sure to assing gene names to df being used
bed_key <- regions$K27promTSS %>%
  select(V1, V2, V3, V10) %>%
  distinct(V1, V2, V3, .keep_all = TRUE)   # guard against duplicate coords

H3K27me3_CS_Results_K27promTSS_vWT <- H3K27me3_CS_Results_K27promTSS_vWT %>%
  left_join(bed_key, by = c("seqnames" = "V1", "start" = "V2", "end" = "V3"))
...
out <- chip_scatter_flex(
  ...
  file_prefix = "H3K36me3_K27promTSS_vs_RNA_cac1",
...
K27lost = subset(H3K27me3_TSS_Promoters, H3K27me3_TSS_Promoters$V10 %in% K27lost$V10)
...
K36lost = subset(H3K27me3_TSS_Promoters, H3K27me3_TSS_Promoters$V10 %in% K36lost$V10)

retainK27 = subset(H3K27me3_TSS_Promoters, ! H3K27me3_TSS_Promoters$V10 %in% K27lost$V10)
retainK36 = subset(H3K27me3_TSS_Promoters, ! H3K27me3_TSS_Promoters$V10 %in% K36lost$V10)
...
#####
K27_K36lost = subset(K27lost, K27lost$V10 %in% K36lost$V10)
K27lost_only = subset(K27lost, ! K27lost$V10 %in% K36lost$V10)
K36lost_only = subset(K36lost, ! K36lost$V10 %in% K27_K36lost$V10)

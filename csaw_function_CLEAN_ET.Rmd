...
#Subset files from bed file function (later in script)
...
gr_A <- GRanges(H3K36me3_no_ash1$seqnames, IRanges(H3K36me3_no_ash1$start + 1, H3K36me3_no_ash1$end))  # BED start +1
...
write.table(H3K36me3_no_ash1, file = "H3K36me3_no_ash1_noK27.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
...
heatmap_to_bed_peaks <- function(
  ...
  min_value         = 0,        # threshold on logFC for direction check
  merge_windows     = TRUE,     # only used in mode="window"; ignored in "region"
  max_gap_bp        = 0,        # merge windows when gaps between them <= this many bp
  min_width_bp      = 0,        # drop merged blocks narrower than this (mode="window")
  zero_based        = TRUE,     # BED = 0-based start by convention
  write_empty_files = FALSE,    # write empty BEDs for strains with no peaks
  create_dataframes = FALSE,    # <-- NEW: also create <prefix>_<strain>_BED objects
  dataframes_env    = .GlobalEnv, # <-- NEW: where to create them
  object_suffix     = "_BED"      # <-- NEW: appended to object name
) {
  ...
  # Parse rownames: last two dot fields are start/end (chr can contain dots)
  ...
  if (any(!is.finite(start)) || any(!is.finite(end)))
    stop("Non-finite starts/ends after parsing rownames.")
  ...
  out_list <- stats::setNames(vector("list", length(lfc_cols)),
                              sub("^logFC_", "", lfc_cols))

  # helper to optionally create an in-memory data.frame per strain
  .maybe_assign_df <- function(df, nm) {
    if (isTRUE(create_dataframes)) {
      assign(nm, df, envir = dataframes_env)
    }
  }
  ...
  for (lc in lfc_cols) {
    ...
    # Keep windows based on requested direction
    keep <- switch(
      direction,
    ...
    if (length(gr_keep) == 0L) {
      empty_df <- data.frame(seqnames=character(), start=integer(), end=integer())
      if (isTRUE(write_empty_files)) {
        bed_file <- file.path(out_dir, sprintf("%s_%s.bed", prefix, strain))
        file.create(bed_file)
      }
      out_list[[strain]] <- empty_df
      .maybe_assign_df(empty_df, paste0(prefix, "_", strain, object_suffix))
      next
    }

    if (mode == "window" && isTRUE(merge_windows)) {
      # Merge adjacent/nearby windows: gap <= max_gap_bp
      merged <- GenomicRanges::reduce(gr_keep, min.gapwidth = as.integer(max_gap_bp) + 1L)
      # Drop tiny merged blocks if requested
      if (min_width_bp > 0L) {
        merged <- merged[IRanges::width(merged) >= as.integer(min_width_bp)]
      }
      gr_out <- merged
    } else {
      # REGION mode (or merge_windows=FALSE): keep original ranges as-is
      gr_out <- gr_keep
    }
    ...
    # Build BED (3 columns)
    df <- as.data.frame(gr_out)[, c("seqnames","start","end")]
    if (isTRUE(zero_based)) df$start <- df$start - 1L
    ...
    # write file
    bed_file <- file.path(out_dir, sprintf("%s_%s.bed", prefix, strain))
    utils::write.table(df, bed_file, sep = "\t", quote = FALSE,
                       row.names = FALSE, col.names = FALSE)

    out_list[[strain]] <- df
    .maybe_assign_df(df, paste0(prefix, "_", strain, object_suffix))
  }
...
peaks_reg <- heatmap_to_bed_peaks(
  ...
  direction     = "positive",
  min_value     = 0.5,
  merge_windows = TRUE,  # ignored in region mode
  zero_based    = TRUE,

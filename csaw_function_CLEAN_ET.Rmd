...
csaw_analysis <- function(
  ...
  fdr_cutoff = 0.05,
  pvalue_cutoff = NULL,
  mask_region = FALSE,               # applies only in region mode
  ...
  run_norm <- function(mod, fit, counts, region_tag, norm_type, gr_for_region = NULL) {
    ...
    for (st in strains) {
      ...
      if (mode == "window") {
        ...
        gr <- merged$regions
        mcols(gr) <- S4Vectors::DataFrame(merged$combined)  # has logFC, PValue, FDR
      } else {
    ...
    # --------- MASKING / KEEP RULES ----------
    if (mode == "window") {
      # Build per-strain significance masks
      strain_names <- sub("^logFC_", "", logfc_cols)

      sig_mat <- matrix(TRUE, nrow(final_df), length(strain_names), dimnames = list(NULL, strain_names))
      if (!is.null(fdr_cutoff))    sig_mat <- sig_mat & (as.matrix(final_df[, sub("^", "FDR_",   strain_names)])    <= fdr_cutoff)
      if (!is.null(pvalue_cutoff)) sig_mat <- sig_mat & (as.matrix(final_df[, sub("^", "PValue_",strain_names)]) <= pvalue_cutoff)

      if (norm_type == "input") {
        # Require positive enrichment vs input for a strain to be considered "passing"
        pos_mat <- as.matrix(final_df[, logfc_cols]) > 0
        pass_mat <- sig_mat & pos_mat
        # Keep a window if ANY strain passes
        keep <- rowSums(pass_mat, na.rm = TRUE) > 0
        heatmap_df <- heatmap_df[keep, , drop = FALSE]
        final_df   <- final_df[keep, , drop = FALSE]
        pass_mat   <- pass_mat[keep, , drop = FALSE]
        # Per-strain zeroing: set logFC to 0 where NOT passing for that strain
        for (st in colnames(pass_mat)) {
          lcol <- paste0("logFC_", st)
          heatmap_df[[lcol]][ !pass_mat[, st] ] <- 0
        }

      } else {
        # vRef (e.g., vWT): sign can be +/-; passing = sig by FDR/P only
        pass_mat <- sig_mat
        keep <- rowSums(pass_mat, na.rm = TRUE) > 0
        heatmap_df <- heatmap_df[keep, , drop = FALSE]
        final_df   <- final_df[keep, , drop = FALSE]
        pass_mat   <- pass_mat[keep, , drop = FALSE]
        # Per-strain zeroing where NOT significant
        for (st in colnames(pass_mat)) {
          lcol <- paste0("logFC_", st)
          heatmap_df[[lcol]][ !pass_mat[, st] ] <- 0
        }

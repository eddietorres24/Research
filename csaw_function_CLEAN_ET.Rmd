...
csaw_analysis <- function(
  ...
  .write_igv_from_heatmap <- function(res_df, hm_df, file_path) {
    ...
    # build key in results and match heatmap order
  ...
  run_norm <- function(mod, fit, counts, region_tag, norm_type, gr_for_region = NULL) {
    ...
    if (mode == "window") {
      if (norm_type == "input") {
        ...
        if (is.null(kept_coords)) {
          warning("No stored vInput rowset for ", key, " — vWT not row-restricted.")
          keep_rows <- rep(TRUE, nrow(heatmap_df))
        } else {
          keep_rows <- rownames(heatmap_df) %in% kept_coords
        }
        for (st in sub("^logFC_", "", logfc_cols)) {
          lcol <- paste0("logFC_",  st)
          fcol <- paste0("FDR_",    st)

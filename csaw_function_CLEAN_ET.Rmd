...
csaw_analysis <- function(
  ...
  fdr_cutoff = 0.05,                 # significance threshold (used in window mode vInput; region if mask_region=TRUE)
  pvalue_cutoff = NULL,              # optional p-value threshold (same usage as above)
  mask_region = FALSE,               # region mode only: if TRUE, apply FDR/P masking to heatmap_df
  write_csv_results = FALSE,         # write csv for results df
  write_csv_heatmap = FALSE,         # write csv for heatmap df
  write_igv = FALSE,                 # write igv tracks from RESULTS df
  ...
  # Build final result/heatmap tables for a given norm type
  run_norm <- function(mod, fit, counts, region_tag, norm_type, gr_for_region = NULL) {
    ...
    for (st in strains) {
      ...
      if (norm_type == "input") {
        ...
        if (!all(needed %in% colnames(design_mat))) {
          ...
          message("    Skipping ", st, " (missing: ", missing, ")"); next
        }
        # (ChIP/Input)_st - (ChIP/Input)_ref  (log2 space)
    ...
    # --- Masking / filtering ---
    if (mode == "window" && norm_type == "input") {
      # Per-strain significance matrices
      F_ok <- matrix(TRUE, nrow(final_df), length(logfc_cols))
      P_ok <- matrix(TRUE, nrow(final_df), length(logfc_cols))
      colnames(F_ok) <- colnames(P_ok) <- logfc_cols

      if (!is.null(fdr_cutoff)) {
        fdr_mat <- as.matrix(final_df[, sub("^logFC_", "FDR_", logfc_cols), drop = FALSE])
        F_ok <- fdr_mat <= fdr_cutoff
        F_ok[is.na(F_ok)] <- FALSE
      }
      if (!is.null(pvalue_cutoff)) {
        p_mat <- as.matrix(final_df[, sub("^logFC_", "PValue_", logfc_cols), drop = FALSE])
        P_ok <- p_mat <= pvalue_cutoff
        P_ok[is.na(P_ok)] <- FALSE
      }

      L_pos <- as.matrix(heatmap_df) > 0  # positivity over input

      # Keep row if ANY strain is significant (by provided cutoffs) AND positive over input
      any_sig_pos <- rowSums((F_ok & P_ok) & L_pos) > 0
      heatmap_df  <- heatmap_df[any_sig_pos, , drop = FALSE]
      final_df    <- final_df[any_sig_pos, , drop = FALSE]
      F_ok        <- F_ok[any_sig_pos, , drop = FALSE]
      P_ok        <- P_ok[any_sig_pos, , drop = FALSE]
      L_pos       <- L_pos[any_sig_pos, , drop = FALSE]

      # Per-strain masking: zero unless (passes FDR if given) AND (passes P if given) AND (logFC>0)
      ok_mat <- (F_ok & P_ok & L_pos)
      H      <- as.matrix(heatmap_df)
      H[!ok_mat] <- 0
      heatmap_df <- as.data.frame(H)
      rownames(heatmap_df) <- paste(final_df$seqnames, final_df$start, final_df$end, sep = ".")

    } else if (mode == "region" && isTRUE(mask_region)) {
      # Optional masking in region mode
      for (st in names(res_list)) {
  ...
  if (mode == "region") {
    ...
    if (!is.null(out_obj$genomewide)) {
      ...
      for (mod in all_mods) {
        ...
        # ---- vInput (establish the allowed/kept window set for this mod) ----
        res_vin <- run_norm(mod, fit, counts, region_tag = nm, norm_type = "input", gr_for_region = NULL)
        vin_keep_keys <- NULL
        if (!is.null(res_vin)) {
          res_df <- res_vin$results
          hm_df  <- res_vin$heatmap
          vin_keep_keys <- rownames(hm_df)  # use as the canonical set for vWT
        ...
        # ---- vWT (restricted to vInput-kept windows) ----
        res_vref <- run_norm(mod, fit, counts, region_tag = nm, norm_type = "ref", gr_for_region = NULL)
        if (!is.null(res_vref)) {
          res_df <- res_vref$results
          hm_df  <- res_vref$heatmap

          if (!is.null(vin_keep_keys)) {
            # Subset *heatmap* rows to exactly the vInput-kept set, in the same order
            ref_keys <- rownames(hm_df)
            keep <- intersect(vin_keep_keys, ref_keys)
    ...
    # If filtered genomewide windows to region sets in out_obj$regions, handle similarly

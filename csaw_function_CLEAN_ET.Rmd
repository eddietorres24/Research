...
# Make BED-like “peaks” from a *_Heatmap_* df
# window mode  -> merge windows when gap <= max_gap_bp, then drop < min_width_bp
# region mode  -> keep given ranges exactly (no merging/min-width filtering)
heatmap_to_bed_peaks <- function(
  ...
  min_value         = 0,
  merge_windows     = TRUE,     # only used when mode="window"
  max_gap_bp        = 0,        # merge if inter-window gap <= this many bp
  min_width_bp      = 0,        # drop merged blocks < this width (bp) in window mode
  zero_based        = TRUE,     # BED start is 0-based
  ...
  # Parse rownames robustly: last two dot fields are start/end (chr can contain dots)
  m  <- regexec("^(.+)\\.([0-9]+)\\.([0-9]+)$", rn)
  mm <- regmatches(rn, m)
  bad <- vapply(mm, length, integer(1L)) != 4L
  if (any(bad)) stop("Bad rowname (chr.start.end) example: ", rn[which(bad)[1L]])
  ...
  # --- tiny helper: deterministic per-chromosome merging by gap ---
  merge_by_gap <- function(chr_vec, start_vec, end_vec, max_gap, min_width) {
    o <- order(chr_vec, start_vec, end_vec)
    chr_o   <- chr_vec[o]
    start_o <- start_vec[o]
    end_o   <- end_vec[o]
    ...
    while (i <= n) {
      ...
      # extend while next interval is on same chr and gap <= max_gap
      while (i <= n && chr_o[i] == cur_chr) {
        gap <- start_o[i] - cur_en - 1L   # inclusive coordinates
        if (gap <= max_gap) {
          # merge -> extend end
          if (end_o[i] > cur_en) cur_en <- end_o[i]
          i <- i + 1L
        } else {
          break
        }
      ...
      # width filter if requested
      if (min_width <= 0L || (cur_en - cur_st + 1L) >= min_width) {
        res_chr <- c(res_chr, cur_chr)
        res_st  <- c(res_st,  cur_st)
        res_en  <- c(res_en,  cur_en)
      }
    }

    if (!length(res_chr)) {
      return(data.frame(seqnames=character(0), start=integer(0), end=integer(0)))
    }
    data.frame(seqnames=res_chr, start=res_st, end=res_en, row.names = NULL, stringsAsFactors = FALSE)
  }
  ...
  for (lc in lfc_cols) {
    ...
    if (!any(keep)) {
      ...
      out_list[[strain]] <- data.frame()
      next
    }

    if (mode == "window" && isTRUE(merge_windows)) {
      df_merged <- merge_by_gap(chr[keep], start[keep], end[keep],
                                max_gap = as.integer(max_gap_bp),
                                min_width = as.integer(min_width_bp))
    } else {
      # region mode or no merge: keep ranges as-is (no min-width filter)
    ...
    # BED output (3 columns)
    if (nrow(df_merged) == 0L) {
      if (write_empty_files) {
        bed_file <- file.path(out_dir, sprintf("%s_%s.bed", prefix, strain))
        file.create(bed_file)
      }
    ...
    if (isTRUE(zero_based)) df_merged$start <- pmax(0L, df_merged$start - 1L)

    bed_file <- file.path(out_dir, sprintf("%s_%s.bed", prefix, strain))
    utils::write.table(df_merged, bed_file, sep = "\t", quote = FALSE,
                       row.names = FALSE, col.names = FALSE)

    out_list[[strain]] <- df_merged
  }

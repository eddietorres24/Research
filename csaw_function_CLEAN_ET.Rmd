...
make_chip_rna_heatmap_simple <- function(
  chip_df,
  rna_list,                                 # named list of RNA dfs; rownames = gene IDs
  gene_col        = "gene_id",
  chip_cols       = NULL,                   # displayed ChIP cols (names like "logFC_cac1"); NULL -> guess from rna_list
  rna_colname     = "log2FoldChange",

  # --- AUTO CLUSTERING (ChIP-based; optional; ignored if manual_clusters is given) ---
  cluster_by       = NULL,                  # strains to define auto clusters (e.g., c("cac1","cac2"))
  collapse_mutants = FALSE,                 # TRUE -> loss/keep/gain in ANY/ALL (ignoring WT)
  fold_cutoff      = NULL,                  # NULL -> derive from chip_zero_band; else numeric
  sort_reference   = c("WT"),               # fallback for auto mode
  # --- GLOBAL SORTING (applies to both auto and manual unless overridden per cluster) ---
  sort_on          = c("chip","rna"),       # what to sort by
  sort_strains     = NULL,                  # which strains to average for sorting (e.g., c("cac1","cac2"))
  sort_decreasing  = TRUE,                  # direction

  # --- MANUAL CLUSTERS (gene_id-based; list order = heatmap order) ---
  # Each entry can be: character vector of gene IDs, a data.frame with a gene column,
  # or a GRanges with mcols holding gene IDs.
  manual_clusters        = NULL,
  manual_labels          = NULL,            # optional pretty labels; defaults to names(manual_clusters) or cluster_#
  manual_keep_unassigned = TRUE,            # include "other" for genes not present in any manual cluster
  manual_sort            = NULL,            # named list of per-cluster sort overrides:
                                            # e.g., list(K36lost = list(on="chip", strains=c("cac1","cac2"), decreasing=FALSE),
                                            #            K27lostonly = list(on="rna",  strains="set7"))
  # appearance
  chip_limits    = c(-2, 2),
  rna_limits     = c(-10, 10),
  chip_zero_band = 1,                       # also used as cutoff if fold_cutoff=NULL
  rna_zero_band  = 2,
  chip_colors    = c("#3B4CC0","white","#B40426"),
  rna_colors     = c("blue","white","red"),
  row_gaps_mm    = 0,                       # 0 => no gaps between clusters
  ...
  stopifnot(is.list(rna_list), length(rna_list) >= 1)
  strains_rna <- names(rna_list); stopifnot(all(nzchar(strains_rna)))
  stopifnot(gene_col %in% names(chip_df))

  # ---------------- Intersection by gene_id ----------------
  genes_chip <- as.character(chip_df[[gene_col]])
  common <- Reduce(intersect, c(list(genes_chip), lapply(rna_list, rownames)))
  chip_df  <- chip_df[match(common, genes_chip), , drop = FALSE]
  rna_list <- lapply(rna_list, function(x) x[common, , drop = FALSE])
  genes <- common
  if (verbose) message("Plottable genes: ", length(genes))

  # ---------------- ChIP matrix (for plotting) ----------------
  all_chip_strains <- sub("^logFC_", "", grep("^logFC_", names(chip_df), value = TRUE))
  if (is.null(chip_cols)) {
    guess <- paste0("logFC_", unique(c(strains_rna, cluster_by)))
    chip_cols <- intersect(guess, names(chip_df))
  }
  stopifnot(length(chip_cols) >= 1, all(chip_cols %in% names(chip_df)))
  chipMat <- as.matrix(chip_df[, chip_cols, drop = FALSE])
  rownames(chipMat) <- genes
  colnames(chipMat) <- sub("^logFC_", "", colnames(chipMat))  # -> strains like "cac1","cac2","set7"
  storage.mode(chipMat) <- "numeric"

  # ---------------- RNA matrix (for plotting) ----------------
  pull_rna <- function(df, g, col) suppressWarnings(as.numeric(df[g, col, drop = TRUE]))
  rnaMat <- do.call(cbind, lapply(strains_rna, function(s) pull_rna(rna_list[[s]], genes, rna_colname)))
  colnames(rnaMat) <- paste0("RNA_", strains_rna)             # -> "RNA_cac1","RNA_cac2","RNA_set7"
  rownames(rnaMat) <- genes

  # =================== CLUSTER ASSIGNMENT ===================

  # Helpers to extract gene IDs from various object types for *manual* clustering
  .genes_from_obj <- function(x) {
    # 1) character vector (already gene IDs)
    if (is.character(x)) return(unique(as.character(x)))
    # 2) data.frame: look for gene_col or common synonyms
    if (is.data.frame(x)) {
      cn <- colnames(x)
      cand <- c(gene_col, "gene_id","gene","Gene","GeneID","gene_name","Name","symbol","V10","V11")
      gcol <- cand[cand %in% cn][1]
      if (!length(gcol)) stop("manual_clusters df has no gene column (looked for: ", paste(cand, collapse=", "), ")")
      return(unique(as.character(x[[gcol]])))
    }
    # 3) GRanges: gene in mcols
    if (methods::is(x, "GRanges")) {
      mc <- S4Vectors::mcols(x)
      cn <- colnames(mc)
      cand <- c(gene_col, "gene_id","gene","Gene","GeneID","gene_name","Name","symbol","V10","V11")
      gcol <- cand[cand %in% cn][1]
      if (!length(gcol)) stop("manual_clusters GRanges has no gene column in mcols (looked for: ", paste(cand, collapse=", "), ")")
      return(unique(as.character(mc[[gcol]])))
    }
    stop("Unsupported manual_clusters element of class: ", paste(class(x), collapse="/"))
  }

  # Build an 'orig' matrix from chip_df for AUTO clustering (independent of what we plot)
  want_strains <- unique(c(sub("^logFC_", "", chip_cols), cluster_by, sort_reference))
  have_strains <- intersect(want_strains, all_chip_strains)
  orig <- as.matrix(chip_df[, paste0("logFC_", have_strains), drop = FALSE])
  colnames(orig) <- have_strains
  rownames(orig) <- genes

  # cutoff for auto mode
  cut <- if (is.null(fold_cutoff)) {
    if (!is.null(chip_zero_band)) {
      if (length(chip_zero_band) == 1) abs(chip_zero_band) else max(abs(chip_zero_band))
    } else 1
  } else as.numeric(fold_cutoff)

  # ---- Sorting helpers (global + per-cluster override) ----
  .score_by <- function(idx, on = sort_on, strains = sort_strains, dec = sort_decreasing) {
    if (on == "chip") {
      cols <- if (is.null(strains)) colnames(chipMat) else intersect(strains, colnames(chipMat))
      if (!length(cols)) cols <- colnames(chipMat)
      sc <- rowMeans(chipMat[idx, cols, drop = FALSE], na.rm = TRUE)
    } else {
      want <- if (is.null(strains)) colnames(rnaMat) else paste0("RNA_", strains)
      cols <- intersect(want, colnames(rnaMat))
      if (!length(cols)) cols <- colnames(rnaMat)
      sc <- rowMeans(rnaMat[idx, cols, drop = FALSE], na.rm = TRUE)
    }
    order(sc, decreasing = dec, na.last = TRUE)
  }

  .score_by_manual <- function(idx, cl_label) {
    if (!is.null(manual_sort) && !is.null(manual_sort[[cl_label]])) {
      ...
      return(.score_by(idx, on = on, strains = st, dec = dec))
    } else {
      return(.score_by(idx, on = sort_on, strains = sort_strains, dec = sort_decreasing))
    }
  }
  ...
  if (use_manual) {
    # -------- MANUAL (gene_id-based) --------
    if (is.null(names(manual_clusters))) {
      names(manual_clusters) <- paste0("cluster_", seq_along(manual_clusters))
    }
    labels <- if (!is.null(manual_labels)) {
      stopifnot(length(manual_labels) == length(manual_clusters)); manual_labels
    } else {
      names(manual_clusters)
    }
    ...
    if (isTRUE(manual_keep_unassigned)) {
      ...
      keep <- !is.na(cl_vec)
      chipMat <- chipMat[keep, , drop = FALSE]
    ...
    # Per-cluster sorting with per-cluster overrides
    ord <- integer(0)
    for (cl in levels(cl_split)) {
      idx <- which(cl_split == cl)
      if (!length(idx)) next
      o <- .score_by_manual(idx, cl)
      ord <- c(ord, idx[o])
    }

    chipMat  <- chipMat[ord, , drop = FALSE]
    rnaMat   <- rnaMat [ord, , drop = FALSE]
    cl_split <- cl_split[ord]

  } else {
    # -------- AUTO (ChIP-based state machine) --------
    if (is.null(cluster_by) || !length(cluster_by)) {
      stop("Auto mode requires 'cluster_by' (e.g., c('cac1','cac2')).")
    }
    state_for <- function(strains) {
      vals <- orig[, strains, drop = FALSE]
      d <- matrix(0L, nrow(vals), ncol(vals), dimnames = dimnames(vals))
    ...
    if (!collapse_mutants) {
      ...
      for (cl in levels(cl_split)) {
        ...
        o <- .score_by(idx)  # global sorting controls
        ord <- c(ord, idx[o])
      }
      chipMat  <- chipMat[ord, , drop = FALSE]
      rnaMat   <- rnaMat [ord, , drop = FALSE]
  ...
  # ---------------- Colors & clamping ----------------
  col_fun <- function(lims, band, cols){
    if (!is.null(band) && lims[1] < 0 && lims[2] > 0) {
      b <- abs(band)[1]
      circlize::colorRamp2(c(lims[1], -b, 0, b, lims[2]),
                           c(cols[1], "white", "white", "white", cols[3]))
    } else if (lims[1] < 0 && lims[2] > 0) {
      circlize::colorRamp2(c(lims[1], 0, lims[2]), c(cols[1], "white", cols[3]))
    } else {
      circlize::colorRamp2(c(lims[1], lims[2]), c(cols[1], cols[3]))
    }
  }
  col_chip <- col_fun(chip_limits, chip_zero_band, chip_colors)
  col_rna  <- col_fun(rna_limits,  rna_zero_band,  rna_colors)

  chipMat <- pmin(pmax(chipMat, chip_limits[1]), chip_limits[2])
  rnaMat  <- pmin(pmax(rnaMat,  rna_limits[1]),  rna_limits[2])

  # ---------------- Heatmaps (no gaps by default) ----------------
  ht_chip <- Heatmap(
    chipMat, name = "ChIP log2FC", col = col_chip,
    cluster_rows = FALSE, cluster_columns = FALSE, show_row_names = FALSE,
    row_split = cl_split, row_gap = unit(row_gaps_mm, "mm")
  )
  ht_rna <- Heatmap(
    rnaMat,  name = "RNA log2FC",  col = col_rna, na_col = "white",
    cluster_rows = FALSE, cluster_columns = FALSE, show_row_names = FALSE,
    row_split = cl_split, row_gap = unit(row_gaps_mm, "mm")
  )
  ...
  invisible(list(
    ...
    rna_matrix    = rnaMat,
    clusters      = cl_split,
    genes_plotted = rownames(chipMat)
  ))
}
...
out <- make_chip_rna_heatmap_simple(
  ...
  manual_clusters = list(
    K36_lost       = K36lost,        # data.frame / GRanges / or character vector of gene_ids
  ...
  sort_decreasing = FALSE,           # negatives at the top
  manual_sort = list(
    "K36 lost"        = list(on = "rna", strains = c("cac1","cac2"), decreasing = TRUE),
    "K27 lost only"   = list(on = "rna",  strains = c("cac1","cac2"), decreasing = TRUE),
  ...
  row_gaps_mm = 0,
  out_dir = "csaw_heatmaps",
  file_prefix = "H3K36me3_vs_RNA_manualClusters",
  save_formats = c("pdf","png"),
  width_in = 8,
  dpi = 300
)
...
###Tool to see what your row order is
hm_rows <- cbind(
  data.frame(
    order_idx = seq_len(nrow(out$chip_matrix)),
    gene      = rownames(out$chip_matrix),
    cluster   = as.character(out$clusters),
    stringsAsFactors = FALSE
  ),
  as.data.frame(out$chip_matrix),   # the ChIP values used for the heatmap
  as.data.frame(out$rna_matrix)     # the RNA values used for the heatmap
)
View(hm_rows)
...
#K27 enrichment
K27_enriched = subset(H3K27me3_promTSS_enriched_log2FC_2_WT_genes_BED, H3K27me3_promTSS_enriched_log2FC_2_WT_genes_BED$V10 %in% H3K27me3_prom_enriched_log2FC_2_WT_genes_BED$V10)

H3K27me3_inCAF1 = subset(K27_enriched, ! K27_enriched$V10 %in% H3K27me3_lost_genes_log2FC_2_cac1_genes_BED$V10 | ! K27_enriched$V10 %in% H3K27me3_lost_genes_log2FC_2_cac2_genes_BED$V10)

H3K27me3_noCAF1 = subset(K27_enriched, K27_enriched$V10 %in% H3K27me3_lost_genes_log2FC_2_cac1_genes_BED$V10 | K27_enriched$V10 %in% H3K27me3_lost_genes_log2FC_2_cac2_genes_BED$V10)
...
K27lost = subset(H3K27me3_CS_Results_K27promTSS_vWT, H3K27me3_CS_Results_K27promTSS_vWT$logFC_cac1 < -1 | H3K27me3_CS_Results_K27promTSS_vWT$logFC_cac2 < -1)
#K27lost = subset(K27lost, K27lost$FDR_cac1 < 0.05 | K27lost$FDR_cac2 < 0.05)
K27lost = subset(regions$K27genes, regions$K27genes$V10 %in% K27lost$gene_id)

K36lost = subset(H3K36me3_Results_K27promTSS_vWT, H3K36me3_Results_K27promTSS_vWT$logFC_cac1 < -1 | H3K36me3_Results_K27promTSS_vWT$logFC_cac2 < -1)
#K36lost = subset(K36lost, K36lost$FDR_cac1 < 0.05 | K36lost$FDR_cac2 < 0.05)
K36lost = subset(regions$K27genes, regions$K27genes$V10 %in% K36lost$gene_id)

retainK27 = subset(regions$K27genes, ! regions$K27genes$V10 %in% K27lost$V10)
retainK36 = subset(regions$K27genes, ! regions$K27genes$V10 %in% K36lost$V10)

write.table(K27lost, file = "K27losstest.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
write.table(K36lost, file = "K36losstest.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE) 

write.table(retainK27, file = "K27retaintest.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
write.table(retainK36, file = "K36retaintest.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE) 

#####
K27lost_only = subset(K27lost, ! K27lost$V10 %in% K36lost$V10)
K27_K36retain = subset(regions$K27genes, ! regions$K27genes$V10 %in% K36lost$V10 & ! regions$K27genes$V10 %in% K27lost$V10)

write.table(K27_K36lost, file = "CAF1_K27_K36_lost.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
write.table(K27lost_only, file = "CAF1_K27_lost_only.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
write.table(K36lost_only, file = "CAF1_K36_lost_only.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)

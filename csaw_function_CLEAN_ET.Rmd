...
metaQC <- read.csv("../csaw_samples_files/cac_csaw_QC_FINAL.csv")
...
regions <- list(K27genes_proms = read.table("../bed_files/K27_genes_prom_100pct_overlap.bed"), K27genes = read.table("../bed_files/K27_genes_stringent.bed"), K27proms = read.table("../bed_files/K27_promoters.bed"), K27promTSS = read.table("../bed_files/K27_prom_TSS.bed"), allgenes = read.table("../bed_files/all_genes_forR.bed"), K27promTSSext = read.table("../bed_files/K27promTSSext.bed"), allgenespromTSS = read.table("../bed_files/allgenes_promTSS.bed"), allgenes_promTSSext = read.table("../bed_files/allgenespromTSSext.bed")) 
...
```{r, QC function}
# ============================================================
# SCRIPT 1: DiffBind-only (ALL samples)
#   - Uses your working logic:
#       * Attempt 1: declared/guessed PeakCaller
#       * Attempt 2: force 'bed'
#       * Attempt 3: cleaned peaks + 'bed'
#   - PCA: by modification (no labels)
#   - Correlation heatmap: colored, no dendrogram, no labels
#   - No ChIPQC here.
# ============================================================
...
.run_diffbind_all <- function(inc, out_dir){
  db_out <- .safe_dir(file.path(out_dir, "DiffBind_ONLY"))
  ...
  dev.off()
  message("  ✅ Correlation heatmap (colored, no dendrogram / no labels): ", corr_pdf)

  invisible(dbObj)
}

run_diffbind_only <- function(metadata, out_dir = "QC_DiffBindOnly") {
  .safe_dir(out_dir)
  ...
  if (nrow(inc) == 0) stop("No valid rows after filtering.")

  message("\n[DiffBind] Running with fallbacks …")
  .run_diffbind_all(inc, out_dir = out_dir)

  message("\nDone (DiffBind only). Outputs in: ", normalizePath(out_dir))
}


# ============================================================
# SCRIPT 2: ChIPQC-only (per modification)
#   - Independent from DiffBind
#   - Forces PeakCaller="bed" to avoid numeric score requirement
#   - Drops samples with 0 reads-in-peaks, then makes one full report per Antibody
#   - No tryCatch wrappers
# ============================================================
suppressPackageStartupMessages({
  library(ChIPQC)
...
.safe_dir_qc <- function(d){
  if (!dir.exists(d)) dir.create(d, recursive = TRUE)
  normalizePath(d, mustWork = FALSE)
}

.is_input_row_qc <- function(df){
  fields <- paste(
    if ("Antibody" %in% names(df)) df$Antibody else "",
    if ("Factor"   %in% names(df)) df$Factor   else "",
    if ("SampleID" %in% names(df)) df$SampleID else "",
...
.make_numeric_replicate_qc <- function(df){
  rep_num <- NA_integer_
  if ("Replicate" %in% names(df)) {
    rep_num <- suppressWarnings(as.integer(gsub("\\D", "", as.character(df$Replicate))))
  }
...
# --- minimal helper to count reads overlapping peaks (to drop zero-read samples) ---
.reads_in_peaks_count_qc <- function(bam, peaks, mapq = 0, max_peaks = 5000){
  if (!file.exists(bam) || !file.exists(peaks)) return(0L)

  # Import peaks (accept narrowPeak/broadPeak/bed/gff/gtf)
  ext <- tolower(tools::file_ext(peaks))
  fmt <- if (ext %in% c("narrowpeak","broadpeak","bed","gff","gff3","gtf")) ext else "bed"
  gr  <- rtracklayer::import(peaks, format = fmt)
  if (length(gr) == 0L) return(0L)
  if (length(gr) > max_peaks) gr <- gr[seq_len(max_peaks)]

  # Detect pairedness safely
  paired <- FALSE
  tp <- try(Rsamtools::testPairedEndBam(bam), silent = TRUE)
  if (!inherits(tp, "try-error") && is.logical(tp) && !is.na(tp)) {
    paired <- isTRUE(tp)
  }

  bf  <- Rsamtools::BamFile(bam, yieldSize = 1e6)
  prm <- Rsamtools::ScanBamParam(
    mapqFilter = mapq,
    flag = Rsamtools::scanBamFlag(isSecondaryAlignment = FALSE)
  )

  # singleEnd must be TRUE (SE) or FALSE (PE); fragments=TRUE for PE
  ...
  as.integer(sum(SummarizedExperiment::assay(se)))
}


run_chipqc_only <- function(metadata, out_dir = "QC_ChIPQCOnly", mapq = 0) {
  .safe_dir_qc(out_dir)
  ...
  meta <- meta %>%
    mutate(
      SampleID  = as.character(SampleID),
      Antibody  = as.character(Antibody),
      bamReads  = trimws(as.character(bamReads)),
      Peaks     = trimws(as.character(Peaks)),
      is_input  = .is_input_row_qc(.),
      bam_ok    = !is.na(bamReads) & bamReads != "" & file.exists(bamReads),
      peak_ok   = !is.na(Peaks)    & Peaks    != "" & file.exists(Peaks)
    )

  inc0 <- meta %>% filter(!is_input & bam_ok & peak_ok)

  message("Total rows: ", nrow(meta))
  message("Included for ChIPQC (pre-screen): ", nrow(inc0))
  if (nrow(inc0) == 0) stop("No valid rows for ChIPQC after filtering.")

  # --- DROP zero reads-in-peaks samples ---
  message("Screening reads-in-peaks (dropping zeros)…")
  rip <- mapply(.reads_in_peaks_count_qc,
                bam   = inc0$bamReads,
                peaks = inc0$Peaks,
                MoreArgs = list(mapq = mapq, max_peaks = 5000))
  inc0$reads_in_peaks <- rip

  drop_zero <- inc0 %>% filter(reads_in_peaks <= 0L)
  inc       <- inc0  %>% filter(reads_in_peaks > 0L)

  if (nrow(drop_zero) > 0) {
    message("Dropped (0 reads in peaks): ", paste(drop_zero$SampleID, collapse = ", "))
    write.csv(drop_zero, file.path(out_dir, "dropped_zero_reads_in_peaks.csv"), row.names = FALSE)
  }
  message("Remaining for ChIPQC: ", nrow(inc))
  if (nrow(inc) == 0) stop("All samples had 0 reads overlapping peaks; nothing to report.")

  # --- one full report per modification (Antibody) ---
  mods <- unique(inc$Antibody)
  message("\n[ChIPQC] Generating reports per modification…")
  for (mod in mods) {
    mod_df  <- inc %>% filter(Antibody == mod)
    if (nrow(mod_df) == 0) next
    mod_out <- .safe_dir_qc(file.path(out_dir, paste0("ChIPQC_", mod)))
    message("  [", mod, "] Samples: ", paste(mod_df$SampleID, collapse = ", "))

    # Minimal sheet for ChIPQC
    chip_sheet <- mod_df %>%
      mutate(
        ...
        Replicate  = .make_numeric_replicate_qc(.),
        PeakCaller = "bed"  # force BED to avoid numeric-score requirement
      ) %>%
      select(SampleID, Tissue, Factor, Condition, Replicate, bamReads, Peaks, PeakCaller)

    chipObj <- ChIPQC(as.data.frame(chip_sheet), mapq = mapq)
    ChIPQCreport(
      chipObj,
...
K27lostK36enriched = H3K36me3_Results_K27promTSS_vInput %>%
  filter(if_any(all_of(c(6, 9, 12, 15, 18, 21)), ~ coalesce(.x > 0, FALSE)))
...
#First, subsetting genes that are actually enriched in the v Input & v WT dfs
...
#################
#Combine dfs and remove duplicate rows

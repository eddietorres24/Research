...
# Flexible scatter: ChIP vs RNA  OR  ChIP vs ChIP (with BOTH-AXES cutoff coloring by Y sign)
chip_scatter_flex <- function(
  mode = c("chip_vs_rna","chip_vs_chip"),

  # --- inputs shared / aligned by gene id ---
  gene_col       = "gene_id",          # gene id column inside ChIP dfs
  ...
  # DEPRECATED (kept only so old calls don't error): skew params are ignored in chip_vs_chip mode
  chip_diff_thresh = NULL,             # deprecated, ignored
  chip_colors_map  = NULL,             # deprecated, ignored
  chip_legend_title = NULL,            # deprecated, ignored

  # New: ChIP vs ChIP coloring based on BOTH axes exceeding |cutoff|, colored by the sign of Y
  chip_abs_cutoff  = 0.75,
  chip_both_axes_colors = c(pos = "red", neg = "blue", other = "gray70"),
  chip_both_axes_legend = "Both axes ≥ |cutoff| (color by sign of Y)",

  # Optional guide lines at ±cutoff on both axes
  ...
  if (mode == "chip_vs_rna") {
    ...
    # New rule: color ONLY points where BOTH |x| and |y| exceed the absolute cutoff.
    # If so, color by the sign of y: red if y >= +cutoff; blue if y <= -cutoff. Otherwise gray.
    cutoff <- abs(chip_abs_cutoff)
    plot_df <- mutate(plot_df,
      pass_both = (abs(x) >= cutoff & abs(y) >= cutoff),
      color_group = case_when(
        pass_both & y >=  cutoff ~ "pos",
        pass_both & y <= -cutoff ~ "neg",
        TRUE                     ~ "other"
      )
    )
    col_vals <- setNames(as.character(chip_both_axes_colors[c("pos","neg","other")]),
                         c("pos","neg","other"))
    legend_title <- chip_both_axes_legend
  }
  ...
  # optional cutoff guides
  if (mode == "chip_vs_chip" && isTRUE(show_cutoff_guides)) {
    cutoff <- abs(chip_abs_cutoff)
    p <- p +
      geom_hline(yintercept = c(-cutoff, cutoff), linetype = guide_linetype, alpha = guide_alpha, inherit.aes = FALSE) +
      geom_vline(xintercept = c(-cutoff, cutoff), linetype = guide_linetype, alpha = guide_alpha, inherit.aes = FALSE)
  }
  ...
  if (!is.null(out_dir)) {
    ...
    default_stem <- if (mode == "chip_vs_rna") {
      ...
      paste0("chip", strain_x, "_vs_chip", strain_y, "_absCut", gsub("\\.", "p", as.character(abs(chip_abs_cutoff))))
    }
  ...
  invisible(list(
    data  = plot_df,   # GeneID, x, y, pass_both, color_group
...
out <- chip_scatter_flex(
  ...
  file_prefix = "K27_K27promTSS_vs_RNA_cac1",
...
out <- chip_scatter_flex(
  ...
  chip_abs_cutoff        = 0.75,
  chip_both_axes_colors  = c(pos = "red", neg = "blue", other = "gray70"),
  chip_both_axes_legend  = NULL,
  show_cutoff_guides     = FALSE,
  out_dir       = "scatter_outputs",
  file_prefix   = "H3K36me3_vs_Kac_set7_promoters_genes_abs075",
  width_in      = 12, height_in = 9, dpi = 600
)
out$gg
...
Kac_genesprom_gain_set7 = subset(Kac_Results_K27genes_proms_vWT, Kac_Results_K27genes_proms_vWT$logFC_set7 > 0.75 & ! Kac_Results_K27genes_proms_vWT$gene_id %in% Kac_genesprom_gain_CAF$gene_id)
...
K4_genes_gain_set7 = subset(H3K4me2_Results_K27genes_vWT, H3K4me2_Results_K27genes_vWT$logFC_set7 > 0.75 & ! H3K4me2_Results_K27genes_vWT$gene_id %in% K4_genes_gain_CAF$gene_id)
...
CAF_K27_K36_Lost <- bind_rows(
  mutate(K27lost, .source = "K27lost"),
  mutate(K36lost, .source = "K36lost")
) %>%

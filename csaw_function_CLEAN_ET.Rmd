...
# regions$allgenes has columns:
# V1 (chrom), V2 (start), V3 (end), V6 (strand), gene_id, etc.

allgenes_promTSS <- regions$allgenes %>%
  mutate(
    # Compute new start (V2) and end (V3) based on strand
    V2_new = if_else(V6 == "+",
                     pmax(V2 - 500L, 1L),         # + strand: [TSS-500, ...]
                     pmax(V3 - 300L, 1L)          # - strand: [..., TSS-300]
    ),
    V3_new = if_else(V6 == "+",
                     V2 + 300L,                   # + strand: ... , TSS+300]
                     V3 + 500L                    # - strand: [TSS+500]
    )
  ) %>%
  # Replace original V2/V3 with the new coordinates
  mutate(V2 = as.integer(V2_new),
         V3 = as.integer(V3_new)) %>%
  select(-V2_new, -V3_new)

# quick sanity checks (optional)
stopifnot(all(allgenes_promTSS$V2 < allgenes_promTSS$V3))   # intervals are valid
# View unique strands and a few rows
# dplyr::glimpse(tss_windows)

...
make_csaw_heatmap_clusterby(
  df = K27_CAF_enriched_bins,
  plot_cols  = c("WT","cac1","cac2","ash1"),
  cluster_by = c("cac1","cac2"),
  split_mode = "auto",
  collapse_mutants = TRUE,      
  fold_cutoff = NULL,
  width_in  = 6.5,
  base_height_in = 1.5,
  per_row_height_in = 0.0005,
  clip_negatives_to_zero = FALSE,
  color_limits     = c(0,5),
  zero_white_band  = 1,
...
K36enrich = subset(H3K36me3_Results_K27proms_vInput, H3K36me3_Results_K27proms_vInput$logFC_WT > 0)
K36scatter = subset(H3K36me3_Results_K27proms_vWT, H3K36me3_Results_K27proms_vWT$gene_id %in% K36enrich$gene_id)
...
K36genepromscatter = subset(H3K36me3_Results_K27genes_proms_vWT, H3K36me3_Results_K27genes_proms_vWT$gene_id %in% K36scatter$gene_id)

K27lostscatter = subset(H3K27me3_CS_Results_K27promTSS_vWT, H3K27me3_CS_Results_K27promTSS_vWT$gene_id %in% K27lost$gene_id)
...
out <- chip_scatter_flex(
  ...
  chip_x_cutoff   = 0.75,
  chip_y_cutoff   = 0.585,
  # Only consider left-of −0.75 on X and above +0.75 on Y:
  chip_cut_use     = c("x_neg","y_pos"),
  chip_cut_colors  = c(both = "red", x_only = "#4daf4a", y_only = "#cab2d6", neither = "gray70"),
  # Draw the same two guides; nothing in the unused quadrants, so axes won’t blow out
  show_cutoff_guides = TRUE,
...
peaks <- heatmap_to_bed_peaks(
  hm_df         = Kac_Heatmap_K27promTSS_vWT,
  out_dir       = "csaw_beds/Kac_vWT",
...
Kac_cac1_suz12_gain_only = subset(Kac_enriched_nochange, Kac_enriched_nochange$logFC_cac1_suz12 > 0.585)
Kac_enriched_nochange_dub = subset(Kac_enriched_nochange, ! Kac_enriched_nochange$gene_id %in% Kac_cac1_suz12_gain_only$gene_id )

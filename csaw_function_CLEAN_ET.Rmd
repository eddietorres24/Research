...
heatmap_to_bed_peaks <- function(
  ...
  max_gap_bp        = 0,        # merge windows with gaps <= this many bp (mode="window")
  ...
  # Parse rownames: take last two dot fields as start/end (chr can contain dots)
  ...
  for (lc in lfc_cols) {
    ...
    # Build BED (3 columns). Use as.data.frame to avoid namespace issues.
...
peaks <- heatmap_to_bed_peaks(
  ...
  out_dir      = "BED_out/H3K36me3_vInput",
...
# Example 1: vInput heatmap, enriched regions per strain across a pre-determined region set (i.e. all genes)
peaks <- heatmap_to_bed_peaks(
  hm_df        = H3K36me3_Heatmap_allgenes_vInput,  # any region-mode heatmap df
  out_dir      = "BED_out/H3K27me3_promoters",
  prefix       = "H3K27me3_promoters",
  mode         = "region",      # <- keeps your given regions as-is
  direction    = "positive",
  min_value    = 0.8,
  merge_windows= FALSE,         # ignored in region mode
  zero_based   = TRUE
)

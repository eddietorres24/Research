...
regions$allgenes_promTSS = subset(regions$allgenes_promTSS, regions$allgenes_promTSS$V10 %in% new$V10)
regions$allgenes_promTSSext = subset(regions$allgenes_promTSSext, regions$allgenes_promTSSext$V10 %in% new$V10)
...
make_csaw_heatmap_clusterby <- function(
  ...
  collapse_mutants = FALSE,     # kept for backwards-compat; ignored in "by_no_signal_per_strain"
  fold_cutoff      = 1,         # used only in "loss_gain_pair" mode
  sort_reference   = c("WT"),
  sort_decreasing  = TRUE,

  # NEW: choose how rows are split
  split_mode       = c("auto", "by_no_signal_per_strain", "loss_gain_pair"),
  # NEW: threshold for "no signal" in vInput-style matrices (<= threshold == no signal)
  signal_threshold = 0,
  ...
  if (split_mode == "auto") {
    split_mode <- if (length(cluster_by) == 2L) "loss_gain_pair" else "by_no_signal_per_strain"
  }
  ...
  # ----- Build row splits -----
  # We'll fill these: 'row_split' (either vector or data.frame) and 'ord' (row order)
  row_split <- NULL
  ord <- seq_len(nrow(M))
  ...
  if (split_mode == "loss_gain_pair") {
    ...
    # simple within-cluster ordering by mean of remaining cluster_by (or sort_reference)
    scores <- rowMeans(orig[, intersect(plot_cols, colnames(orig)), drop = FALSE], na.rm = TRUE)
    o <- order(row_split, -scores)  # cluster blocks first, then by score
    ord <- ord[o]
    row_split <- row_split[o]

  } else if (split_mode == "by_no_signal_per_strain") {
    # For each strain s in cluster_by, split rows into "no signal in s" (<= threshold) vs "signal in s" (> threshold).
    # This produces nested splits in the order of 'cluster_by'.
    ...
    # Order rows lexicographically by those factors, then within each terminal bin by mean signal across plot_cols
    # (higher signal first unless sort_decreasing = FALSE)
    # Build a matrix of integer codes for fast ordering:
    code_mat <- sapply(split_df, as.integer)
    # fallback score: mean across available plot cols
    score <- rowMeans(orig[, intersect(plot_cols, colnames(orig)), drop = FALSE], na.rm = TRUE)
    # composite order: by each split column then by score
    o <- do.call(order, c(as.data.frame(code_mat), list(order(score, decreasing = sort_decreasing, na.last = TRUE))))
    ord <- ord[o]
    row_split <- split_df[o, , drop = FALSE]
  }

  # Reorder matrix and splits
  M <- M[ord, , drop = FALSE]
...
make_csaw_heatmap_clusterby(
  ...
  zero_white_band = 1,          # => loss ≤ -1, gain ≥ +1; keep in between
...
make_csaw_heatmap_clusterby(
  ...
  plot_cols    = c("WT","cac1","cac2","cac3","set7"),
  cluster_by   = c("WT","cac1","cac2","cac3"),  # WT included but collapse ignores it when deciding loss/keep/gain
  collapse_mutants = FALSE,               # yields: loss in any (cac1|cac2), kept in all, gain in any
  fold_cutoff  = 1,
  ...
  color_limits = c(0, NA),
  out_stem = "H3K27me3_vInput_CAF-1"
)
...
make_csaw_heatmap_clusterby(
  df         = H3K27me3_CS_Heatmap_K27promTSS_vInput,
  plot_cols  = c("WT","cac1","cac2","cac3","set7"),
  cluster_by = c("WT","cac1","cac2","cac3","set7"),  # order of sequential splits
  split_mode = "by_no_signal_per_strain",
  signal_threshold = 0,           # <= 0 is "no signal"
  colors       = c("white","#006400"),
  color_limits = c(0, NA),
  out_stem     = "H3K27me3_vInput_CAF-1"
)
...
out <- chip_scatter_flex(
  mode        = "chip_vs_rna",
  chip_df_x   = Kac_Results_K27promTSS_vWT,   
  strain_x    = "set7",
  ...
  file_prefix = "Kac_vs_RNA_set7_K27_promTSS",
...
out <- chip_scatter_flex(
  ...
  strain_x      = "cac2",
  chip_df_y     = Kac_Results_K27promTSS_vWT,
  strain_y      = "cac2",
...
K36lost_set7 = subset(H3K36me3_Results_K27promTSS_vWT, H3K36me3_Results_K27promTSS_vWT$logFC_set7 < -0.75)
...
Kac_enriched <- Kac_Results_K27promTSS_vInput %>%
  filter(if_any(starts_with("logFC"),
                ~ coalesce(.x > 0, FALSE)))
K4_enriched <- H3K4me2_Results_K27genes_vInput %>%
  filter(if_any(starts_with("logFC"),
                ~ coalesce(.x > 0, FALSE)))

Kac_enricheddub <- Kac_Results_K27promTSS_vInput %>%
  filter(if_any(starts_with("logFC"),
                ~ coalesce(.x > 0, FALSE)))
...
Kac_enrichedvWT = subset(Kac_Results_K27promTSS_vWT, Kac_Results_K27promTSS_vWT$gene_id %in% Kac_enriched$gene_id)
K4_enrichedvWT = subset(H3K4me2_Results_K27genes_vWT, H3K4me2_Results_K27genes_vWT$gene_id %in% K4_enriched$gene_id)
Kac_enrichedvWTdub = subset(Kac_Results_K27promTSS_vWT, Kac_Results_K27promTSS_vWT$gene_id %in% Kac_enricheddub$gene_id)
...
Kac_prom_gain_CAF = subset(Kac_enrichedvWT, Kac_enrichedvWT$logFC_cac1 > 0.75 | Kac_enrichedvWT$logFC_cac2 > 0.75)
Kac_prom_gain_cac1 = subset(Kac_enrichedvWT, Kac_enrichedvWT$logFC_cac1 > 0.75)
Kac_prom_gain_cac2 = subset(Kac_enrichedvWT, Kac_enrichedvWT$logFC_cac2 > 0.75)
Kac_prom_gain_set7 = subset(Kac_enrichedvWT, Kac_enrichedvWT$logFC_set7 > 0.75)
Kac_prom_gain_suz12 = subset(Kac_enrichedvWT, Kac_enrichedvWT$logFC_suz12 > 0.75)
Kac_prom_gain_suz12_noCAF = subset(Kac_enrichedvWT, Kac_enrichedvWT$logFC_suz12 > 0.75 & ! Kac_enrichedvWT$gene_id %in% Kac_prom_gain_CAF$gene_id)
Kac_prom_gain_set7_noCAF = subset(Kac_enrichedvWT, Kac_enrichedvWT$logFC_set7 > 0.75 & ! Kac_enrichedvWT$gene_id %in% Kac_prom_gain_CAF$gene_id)
Kac_enriched_nochange = subset(Kac_enrichedvWT, ! Kac_enrichedvWT$gene_id %in% Kac_prom_gain_CAF$gene_id & ! Kac_enrichedvWT$gene_id %in% Kac_prom_gain_suz12$gene_id)
Kac_enriched_nochangedub = subset(Kac_enrichedvWT, ! Kac_enrichedvWT$gene_id %in% Kac_prom_gain_CAF$gene_id & ! Kac_enrichedvWT$gene_id %in% Kac_prom_gain_suz12$gene_id)
Kac_uneriched = subset(Kac_Results_K27promTSS_vWT, ! Kac_Results_K27promTSS_vWT$gene_id %in% Kac_enriched$gene_id)

Kac_Gain_CAF1 = subset(regions$K27genes, regions$K27genes$gene_id %in% Kac_prom_gain_CAF$gene_id)
...
Kac_no_change = subset(regions$K27genes, regions$K27genes$gene_id %in% Kac_nochange$gene_id)
Kac_unerich = subset(regions$K27genes, regions$K27genes$gene_id %in% Kac_uneriched$gene_id)

write.table(Kac_Gain_CAF1, file = "Kac_Gain_CAF1.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
...
K4_nochange = subset(K4_enrichedvWT, ! K4_enrichedvWT$gene_id %in% K4_genes_gain_CAF$gene_id & ! K4_enrichedvWT$gene_id %in% K4_genes_gain_suz12$gene_id)
...
write.table(K4_Gain_CAF, file = "K4_Gain_CAF1.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)

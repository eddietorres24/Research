...
ash1_05 <- results(dds2, alpha=alpha, contrast=c("condition", "ash1", "WT"))

#Taking into account adjusted p-value and log2FC cutoff of 1.5
#(log2FoldChange > 1 | log2FoldChange < -1)
cac1_05 <- subset(cac1_05, padj < 0.05 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
cac2_05 <- subset(cac2_05, padj < 0.05 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
cac3_05 <- subset(cac3_05, padj < 0.05 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
naf1_05 <- subset(naf1_05, padj < 0.05 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
naf2_05 <- subset(naf2_05, padj < 0.05 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
asf1_05 <- subset(asf1_05, padj < 0.05 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
atrx_05 <- subset(atrx_05, padj < 0.05 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
set7_05 <- subset(set7_05, padj < 0.05 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
cac1_cac2_05 <- subset(cac1_cac2_05, padj < 0.05 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
cac1_suz12_05 <- subset(cac1_suz12_05, padj < 0.05 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
ash1_05 <- subset(ash1_05, padj < 0.05 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
...
write.csv(as.data.frame(ash1_05), file = "ash1_05.csv")
...
ash1_01 <- results(dds2, alpha=alpha, contrast=c("condition", "asf1", "WT"))

#Taking into account adjusted p-value
cac1_01 <- subset(cac1_01, padj < 0.01 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
cac2_01 <- subset(cac2_01, padj < 0.01 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
cac3_01 <- subset(cac3_01, padj < 0.01 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
naf1_01 <- subset(naf1_01, padj < 0.01 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
naf2_01 <- subset(naf2_01, padj < 0.01 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
asf1_01 <- subset(asf1_01, padj < 0.01 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
atrx_01 <- subset(atrx_01, padj < 0.01 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
set7_01 <- subset(set7_01, padj < 0.01 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
cac1_cac2_01 <- subset(cac1_cac2_01, padj < 0.01 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
cac1_suz12_01 <- subset(cac1_suz12_01, padj < 0.01 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
ash1_01 <- subset(ash1_01, padj < 0.01 & (log2FoldChange > 1.5 | log2FoldChange < -1.5 ))
...
write.csv(as.data.frame(asf1_01), file = "ash1_01.csv")
```
...
cac1_DEG_05 <- read.csv("./cac1_05.csv", header= TRUE, row.names = 1)
...
cac1_DEG_01 <- read.csv("./cac1_01.csv", header= TRUE, row.names = 1)
...
cac1_UP_01 <- subset(cac1_DEG_01, cac1_DEG_01$log2FoldChange > 1.5)
cac1_DOWN_01 <- subset(cac1_DEG_01, cac1_DEG_01$log2FoldChange < -1.5)
cac2_UP_01 <- subset(cac2_DEG_01, cac2_DEG_01$log2FoldChange > 1.5)
cac2_DOWN_01 <- subset(cac2_DEG_01, cac2_DEG_01$log2FoldChange < -1.5)
...
cac1_up_K27 <- subset(cac1_up_genes_01, cac1_up_genes_01$VALUE %in% Prc2targets$V11)
cac2_up_K27 <- subset(cac2_up_genes_01, cac2_up_genes_01$VALUE %in% Prc2targets$V11)
...
venn.diagram(
  x = list(cac1UP$NCU, cac2_UP$NCU, cac3_UP$NCU),
...
cacUPcombine <- bind_rows(
  mutate(cac1UP, .source = "cac1UP"),
  mutate(cac2UP, .source = "cac2UP")
) %>%
...
#Venn Diagram w/ All Upregulated genes in CAF-1 mutants
venn.diagram(
  x = list(cacUPcombine$NCU, K27lost$V10, K36lost$V10),
...
#Make euler plot inputing values gathered from venn.diagram(). This will provide a proportional venn-diagram
CAF1_up_fit = euler(c("A" = 236, "B" = 22, "C" = 2, "A&B" = 115, "A&C" = 9, "B&C" = 8, "A&B&C" = 30))
CAF1_up_eulerplot <- plot(CAF1_up_fit)

#Save file
ggsave(filename = "CAF-1_Up_K27_K36_loss.png", plot = CAF1_up_eulerplot, width = 4, height = 4, dpi = 600)
```
...
track = combine %>% select("SequenceID", "FeatureStart", "FeatureEnd", "VALUE", "asf1_05.csv", "atrx_05.csv", "cac1_05.csv", "cac2_05.csv", "cac3_05.csv", "naf1_05.csv", "naf2_05.csv", "set7_05.csv", "cac1_05.csv", "cac1_cac2_05.csv", "cac1_suz12_05.csv")

...
export_deseq_csvs <- function(
  ...
  write_deg_csvs <- function(strain, alpha, reslist) {
    ...
    ## 4) DOWN results  ⬅️ (already existed)
    if (nrow(reslist$dn)) {
  ...
  ## ---- CAF-1 combos: pairwise (cac1 & cac2) & triple (cac1 & cac2 & cac3) ----
  # Previously only UP/DE were written. Now we also export DOWN intersections.
  ...
  if (have_caf1) {
    for (alpha in alphas) {
      ...
      dn_sets <- lapply(caf1, function(s) deg_map[[key(s)]]$dn_ids)   # ⬅️ NEW
      ...
      ## NEW: DOWN intersections
      safe_write(bed_from_ids(intersect(dn_sets[[1]], dn_sets[[2]])),
                 sprintf("cac1_cac2__alpha%s__DOWN_intersection.csv", a))
      safe_write(bed_from_ids(setdiff(intersect(dn_sets[[1]], dn_sets[[2]]), prc2_ids)),
                 sprintf("cac1_cac2__alpha%s__nonK27_DOWN_intersection.csv", a))
      ...
      ## NEW: triple DOWN intersections
      safe_write(bed_from_ids(Reduce(intersect, dn_sets)),
                 sprintf("cac1_cac2_cac3__alpha%s__DOWN_intersection.csv", a))
      safe_write(bed_from_ids(setdiff(Reduce(intersect, dn_sets), prc2_ids)),
                 sprintf("cac1_cac2_cac3__alpha%s__nonK27_DOWN_intersection.csv", a))
    }
  } else {
...
p <- ggplot(plot_df, aes(FoldEnrich, Label)) +
  ...
  labs(x = "Fold Enrichment", y = NULL,
       title = expression("GO Biological Process Enrichment (cac-1 cac-2 DOWN DE)")) +

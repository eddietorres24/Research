...
Data_Doubles = Averaged_Orderd_KO_data [,c(1,3,4,10,12,11)]
...
# Define factor order and formatted x-axis labels
label_order <- c("WT", "cac-1", "cac-2", "cac-1_cac-2", "suz-12", "cac-1_suz-12")
formatted_labels <- c(
  "WT",
  expression(italic("\u0394cac-1")),
  expression(italic("\u0394cac-2")),
  expression(italic("\u0394cac-1_cac-2")),
...
CAF1_K27_lost_UP_fit = euler(c("A" = 53, "B" = 145, "A&B" = 147))
...
venn.diagram(
  x = list(K27lostash1$gene_id, CAF_K27_K36_Lost$gene_id),
...
CAF1_ASH1_K27lost_fit = euler(c("A" = 286, "B" = 53, "A&B" = 176))
...
#K27 genes upregulated CAF1 K36 lost
venn.diagram(
  x = list(K36lost$gene_id, cacupcombinek27$Attributes),
  category.names = c("K36lost", "CAF UP"),
  filename = 'K36lost_CAF_UP_genes.png',
  col=c("#8da0cb","#fc8d62"),
  fill = c(alpha("#8da0cb",0.25), alpha("#fc8d62",0.25)),
  output=TRUE)

K36lost_CAF_UP_fit = euler(c("A" = 148, "B" = 36, "A&B" = 256))
K36lost_CAF_UP_eulerplot <- plot(K36lost_CAF_UP_fit)
ggsave(filename = "K36lost_CAF_UP.pdf", plot = K36lost_CAF_UP_eulerplot, width = 4, height = 4, dpi = 600)

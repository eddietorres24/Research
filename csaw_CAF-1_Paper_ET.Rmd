...
K4.chip.Filteredres.df <- data.frame(seqnames=seqnames(WTvI.K4.out.ranges),
  ...
  logFC_cac2_compvInput_K4me2 = cac2_compvInput.K4.out.ranges$rep.logFC,
  PValue_cac2_compvInput_K4me2 = cac2_compvInput.K4.out.ranges$PValue,
  FDR_cac2_compvInput_K4me2 = cac2_compvInput.K4.out.ranges$FDR,
  logFC_rtt106vInput_K4me2 = rtt106vInput.K4.out.ranges$rep.logFC,
  PValue_rtt106vInput_K4me2 = rtt106vInput.K4.out.ranges$PValue,
  FDR_rtt106vInput_K4me2 = rtt106vInput.K4.out.ranges$FDR,
  logFC_cac1_rtt106vInput_K4me2 = cac1_rtt106vInput.K4.out.ranges$rep.logFC,
  PValue_cac1_rtt106vInput_K4me2 = cac1_rtt106vInput.K4.out.ranges$PValue,
  FDR_cac1_rtt106vInput_K4me2 = cac1_rtt106vInput.K4.out.ranges$FDR)
...
#I want to subset the data frame to get rid of regions that aren't actually K27 enirched/depleted in any of the mutants. I will do this by eliminating regions that are < 0.1 in all 3 mutants
K27_FilteredHeatmap_mat <- subset(K27_FilteredHeatmap_mat, K27_FilteredHeatmap_mat[, 1] >= 0.101 | K27_FilteredHeatmap_mat[, 2] >= 0.101 | K27_FilteredHeatmap_mat[, 3] >= 0.101 | K27_FilteredHeatmap_mat[, ] >= 0.101)

#throwing our regions that are > 0.1 in set-7
...
#testing making the 0.1 values 0
K27_FilteredHeatmap_mat_sort[K27_FilteredHeatmap_mat_sort == 0.1] <- 0
...
pdf("./H3K27me3_peaks_WT_v_CAF-1_Paper.pdf", height = 9)
# Draw the heatmap
...
ATAC_FilteredHeatmap_df <- dplyr::select(ATAC.chip.Filteredres.df_SigOnly,7,10,13,16)
...
K27_FilteredHeatmap_mat <- as.matrix(K27_FilteredHeatmap_df)
quantile(K27_FilteredHeatmap_mat, c(.01, .50, .75, .95, .98 , .995)) 
max <- quantile(K27_FilteredHeatmap_mat, 0.999) # identify the 98th percentile value and assign this to max
K27_FilteredHeatmap_mat[K27_FilteredHeatmap_mat>max] = max    #replace any value larger than the 98th percentile value with the 98th

library(circlize)
col_fun<- colorRamp2(c(0, quantile(K27_FilteredHeatmap_mat, 0.999)), c("white", "darkgreen"))
col_fun(seq(0, quantile(K27_FilteredHeatmap_mat, 0.999), length = 20))
...
pdf("./K27_K4_regions.pdf", height = 9)
# Draw the heatmap
draw(K27_Filtered_hm)
# Close the device to save the file
dev.off()
...
#get rid on non-K4 regions that got through
K4_FilteredHeatmap_mat <- subset(K4_FilteredHeatmap_mat, K4_FilteredHeatmap_mat[, 1] >= 0.101 | K4_FilteredHeatmap_mat[, 2] >= 0.101 | K4_FilteredHeatmap_mat[, 3] >= 0.101 | K4_FilteredHeatmap_mat[, 4] >= 0.101)


#column color
library(circlize)
col_fun<- colorRamp2(c(0, quantile(K4_FilteredHeatmap_mat, 0.98)), c("white", "#f655f1"))
col_fun(seq(0, quantile(K4_FilteredHeatmap_mat, 0.98), length = 20))


###SORTING
#subsetting to visualize gains/losses of K4 across all 3 mutants
noCacS7 <- subset(K4_FilteredHeatmap_mat, ! K4_FilteredHeatmap_mat[, 1] <= 0.1 & K4_FilteredHeatmap_mat[, 2] <= 0.1 & K4_FilteredHeatmap_mat[, 3] <= 0.1 & K4_FilteredHeatmap_mat[, 4] <= 0.1 )

noS7 <- subset(K4_FilteredHeatmap_mat, ! K4_FilteredHeatmap_mat[, 1] <= 0.1 & (! K4_FilteredHeatmap_mat[, 2] <= 0.1 | ! K4_FilteredHeatmap_mat[, 3] <= 0.1) & K4_FilteredHeatmap_mat[, 4] <= 0.1 )

nocac2 <- subset(K4_FilteredHeatmap_mat, ! K4_FilteredHeatmap_mat[, 1] <= 0.1 & ! K4_FilteredHeatmap_mat[, 2] <= 0.1 & K4_FilteredHeatmap_mat[, 3] <= 0.1 & ! K4_FilteredHeatmap_mat[, 4] <= 0.1 )

nocac1 <- subset(K4_FilteredHeatmap_mat, ! K4_FilteredHeatmap_mat[, 1] <= 0.1 & K4_FilteredHeatmap_mat[, 2] <= 0.1 & ! K4_FilteredHeatmap_mat[, 3] <= 0.1 & ! K4_FilteredHeatmap_mat[, 4] <= 0.1 )

noCacs <- subset(K4_FilteredHeatmap_mat, ! K4_FilteredHeatmap_mat[, 1] <= 0.1 & K4_FilteredHeatmap_mat[, 2] <= 0.1 & K4_FilteredHeatmap_mat[, 3] <= 0.1 & ! K4_FilteredHeatmap_mat[, 4] <= 0.1 )
  
inall <-subset(K4_FilteredHeatmap_mat, ! K4_FilteredHeatmap_mat[, 1] <= 0.1 & ! K4_FilteredHeatmap_mat[, 2] <= 0.1 & ! K4_FilteredHeatmap_mat[, 3] <= 0.1 & ! K4_FilteredHeatmap_mat[, 4] <= 0.1 )
  
CacsS7 <- subset(K4_FilteredHeatmap_mat, K4_FilteredHeatmap_mat[, 1] <= 0.1 & ! K4_FilteredHeatmap_mat[, 2] <= 0.1 & ! K4_FilteredHeatmap_mat[, 3] <= 0.1 & ! K4_FilteredHeatmap_mat[, 4] <= 0.1 )

Cacs <- subset(K4_FilteredHeatmap_mat, K4_FilteredHeatmap_mat[, 1] <= 0.1 & ! K4_FilteredHeatmap_mat[, 2] <= 0.1 & ! K4_FilteredHeatmap_mat[, 3] <= 0.1 & K4_FilteredHeatmap_mat[, 4] <= 0.1 )

cac1o <- subset(K4_FilteredHeatmap_mat, K4_FilteredHeatmap_mat[, 1] <= 0.1 & ! K4_FilteredHeatmap_mat[, 2] <= 0.1 & K4_FilteredHeatmap_mat[, 3] <= 0.1 & K4_FilteredHeatmap_mat[, 4] <= 0.1 )

cac2o <- subset(K4_FilteredHeatmap_mat, K4_FilteredHeatmap_mat[, 1] <= 0.1 & K4_FilteredHeatmap_mat[, 2] <= 0.1 & ! K4_FilteredHeatmap_mat[, 3] <= 0.1 & K4_FilteredHeatmap_mat[, 4] <= 0.1 )

cac1S7 <- subset(K4_FilteredHeatmap_mat, K4_FilteredHeatmap_mat[, 1] <= 0.1 & ! K4_FilteredHeatmap_mat[, 2] <= 0.1 & K4_FilteredHeatmap_mat[, 3] <= 0.1 & ! K4_FilteredHeatmap_mat[, 4] <= 0.1 )

cac2S7 <- subset(K4_FilteredHeatmap_mat, K4_FilteredHeatmap_mat[, 1] <= 0.1 & K4_FilteredHeatmap_mat[, 2] <= 0.1 & ! K4_FilteredHeatmap_mat[, 3] <= 0.1 & ! K4_FilteredHeatmap_mat[, 4] <= 0.1 )

S7 <- subset(K4_FilteredHeatmap_mat, K4_FilteredHeatmap_mat[, 1] <= 0.1 & K4_FilteredHeatmap_mat[, 2] <= 0.1 & K4_FilteredHeatmap_mat[, 3] <= 0.1 & ! K4_FilteredHeatmap_mat[, 4] <= 0.1 )


#ordering the subsets so that enrichment is ordered from highest to lowest in each one
noCacS7_order <-order(noCacS7[, 1], decreasing = TRUE)
noS7_order <- order(rowMeans(noS7[, 1:3]), decreasing = TRUE)
nocac2_order <- order(rowMeans(nocac2[, 1&2&4]), decreasing = TRUE)
nocac1_order <- order(rowMeans(nocac1[, 1&3&4]), decreasing = TRUE)
noCacs_order <- order(rowMeans(noCacs[, 1&4]), decreasing = TRUE)
inall_order <- order(rowMeans(inall[, 1:4]), decreasing = TRUE)
CacsS7_order <- order(rowMeans(CacsS7[, 2:4]), decreasing = TRUE)
Cacs_order <- order(rowMeans(Cacs[, 2:3]), decreasing = TRUE)
cac1o_order <- order(cac1o[, 2], decreasing = TRUE)
cac2o_order <- order(cac2o[, 3], decreasing = TRUE)
cac1S7_order <- order(rowMeans(cac1S7[, 2&4]), decreasing = TRUE)
cac2S7_order <- order(rowMeans(cac2S7[, 3&4]), decreasing = TRUE)
S7_order <- order(S7[, 4], decreasing = TRUE)

#I now have to sort each new subset df separately based on enrichment in desired sample (WT or mutant), then I have to extract the original sorted # to properly sort them on the final heatmap. This is probably a backwards way to do this.
noCacS7_sort <- noCacS7[noCacS7_order, ]
noS7_sort <- noS7[noS7_order, ]
nocac2_sort <- nocac2[nocac2_order, ]
nocac1_sort <- nocac1[nocac1_order, ]
noCacs_sort <- noCacs[noCacs_order, ]
inall_sort <- inall[inall_order, ]
CacsS7_sort <- CacsS7[CacsS7_order, ]
Cacs_sort <- Cacs[Cacs_order, ]
cac1o_sort <- cac1o[cac1o_order, ]
cac2o_sort <- cac2o[cac2o_order, ]
cac1S7_sort <- cac1S7[cac1S7_order, ]
cac2S7_sort <- cac2S7[cac2S7_order, ]
S7_sort <- S7[S7_order, ]

#need rownames of each subset in order
forheat1 <- rownames(noCacS7_sort)
forheat2 <- rownames(noS7_sort)
forheat3 <- rownames(nocac2_sort)
forheat4 <- rownames(nocac1_sort)
forheat5 <- rownames(noCacs_sort)
forheat6 <- rownames(inall_sort)
forheat7 <- rownames(cac1o_sort)
forheat8 <- rownames(cac2o_sort)
forheat9 <- rownames(Cacs_sort)
forheat10 <- rownames(cac1S7_sort)
forheat11 <- rownames(cac2S7_sort)
forheat12 <- rownames(CacsS7_sort)
forheat13 <- rownames(S7_sort)
  
finalorder <- c(forheat1, forheat2, forheat3, forheat4, forheat5, forheat6, forheat7, forheat8, forheat9, forheat10, forheat11, forheat12, forheat13)
...
library(ComplexHeatmap)
K4_Filtered_hm <- Heatmap(K4_FilteredHeatmap_mat_sort, col = col_fun, cluster_columns = F, show_row_names = FALSE, show_column_dend = FALSE, show_row_dend = FALSE, row_order = order(as.numeric(gsub("row", "", rownames(K4_FilteredHeatmap_mat_sort)))) , heatmap_legend_param = list(title = "H3K4me2"))
pdf("./K4_WTvmutants_sort_3.pdf", height = 9)
# Draw the heatmap
draw(K4_Filtered_hm)
# Close the device to save the file
dev.off()

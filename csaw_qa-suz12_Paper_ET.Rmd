...
Allpeaks <- read.table(file="./bed_files/All_qa_peaks_sorted.bed")
...
#cac-1_CS
FC_cac1_CSvInput.K27.contrast <- makeContrasts(K27vcac1_CS=cac1_CS.H3K27me3-cac1.input,levels=design.mat)
...
#cac-2_CS
FC_cac2_CSvInput.K27.contrast <- makeContrasts(K27vcac2_CS=cac2_CS.H3K27me3-cac2.input,levels=design.mat)
...
#cac-3_CS
FC_cac3_CSvInput.K27.contrast <- makeContrasts(K27vcac3_CS=cac3_CS.H3K27me3-cac3.input,levels=design.mat)
...
FC_cac1_abcvInput.K27.contrast <- makeContrasts(K27vcac1_abc=cac1_abc.H3K27me3-cac1.input,levels=design.mat)
...
FC_cac2_abcvInput.K27.contrast <- makeContrasts(K27vcac2_abc=cac2_abc.H3K27me3-cac2.input,levels=design.mat)
...
FC_cac3_abcvInput.K27.contrast <- makeContrasts(K27vcac3_abc=cac3_abc.H3K27me3-cac3.input,levels=design.mat)
...
K27_FilteredHeatmap_df <- dplyr::select(K27.chip.Filteredres.df_SigOnly,7,10,13,16,19,22,25,28,31,34,37,40,43,46,49)
...
K27_FilteredHeatmap_mat <- subset(K27_FilteredHeatmap_mat, K27_FilteredHeatmap_mat[, 1] > 0 | K27_FilteredHeatmap_mat[, 2] > 0)

#throwing our regions that are > 0.1 in suz12 (if any got through)
K27_FilteredHeatmap_mat <- subset(K27_FilteredHeatmap_mat, K27_FilteredHeatmap_mat[, 9] <= 0)

# #I only want to look at qa-suz12
K27_FilteredHeatmap_mat_map <- K27_FilteredHeatmap_mat[,1:8]

#cluster into areas that recover vs. not
recover <- subset(K27_FilteredHeatmap_mat, rowSums(K27_FilteredHeatmap_mat[, 3:7] > 0) > 0)

no_recover <- subset(K27_FilteredHeatmap_mat, rowSums(K27_FilteredHeatmap_mat[, 3:7] <= 0) == 5)

recover_order <-order(recover[, 1], decreasing = TRUE)
...
# #order based on WT
# row_order <- order(K27_FilteredHeatmap_mat[, 1], decreasing = TRUE)
# K27_FilteredHeatmap_mat_sort <- K27_FilteredHeatmap_mat_map[row_order, ]
...
pdf("./H3K27me3_peaks_qa-suz12_TEST.pdf", height = 9)
# Draw the heatmap
...
#cac-1_CS
FC_cac1_CSvInput.All.contrast <- makeContrasts(K27vcac1_CS=cac1_CS.H3K27me3-cac1.input,levels=design.mat)
...
#cac-2_CS
FC_cac2_CSvInput.All.contrast <- makeContrasts(K27vcac2_CS=cac2_CS.H3K27me3-cac2.input,levels=design.mat)
...
#cac-3_CS
FC_cac3_CSvInput.All.contrast <- makeContrasts(K27vcac3_CS=cac3_CS.H3K27me3-cac3.input,levels=design.mat)
...
FC_cac1_abcvInput.All.contrast <- makeContrasts(K27vcac1_abc=cac1_abc.H3K27me3-cac1.input,levels=design.mat)
...
FC_cac2_abcvInput.All.contrast <- makeContrasts(K27vcac2_abc=cac2_abc.H3K27me3-cac2.input,levels=design.mat)
...
FC_cac3_abcvInput.All.contrast <- makeContrasts(K27vcac3_abc=cac3_abc.H3K27me3-cac3.input,levels=design.mat)
...
All_FilteredHeatmap_df <- dplyr::select(All.chip.Filteredres.df_SigOnly,7,10,13,16,19,22,25,28,31,34,37,40,43,46,49)
...
#I want to subset the data frame to get rid of regions that aren't actually K27 enirched/depleted in any of the mutants. I will do this by eliminating regions that are < 0 in both WT samples
All_FilteredHeatmap_mat_map <- subset(All_FilteredHeatmap_mat, rowSums(All_FilteredHeatmap_mat[, 1:7] > 0) > 0)

#throwing our regions that are > 0.1 in suz12 (if any got through)
All_FilteredHeatmap_mat_map <- subset(All_FilteredHeatmap_mat_map, All_FilteredHeatmap_mat_map[, 9] <= 0)

#I want to look at qa-suz12 only
All_FilteredHeatmap_mat_map <- All_FilteredHeatmap_mat_map[,c(1:7)]
...
All_FilteredHeatmap_mat_map <- All_FilteredHeatmap_mat_map[,c(1,7,10:12)]
...
#Plot
All_Filtered_hm <- Heatmap(All_FilteredHeatmap_mat_sort, col = col_fun, cluster_columns = FALSE, show_row_names = FALSE, row_order = order(as.numeric(gsub("row", "", rownames(All_FilteredHeatmap_mat_sort)))), show_column_dend = FALSE, show_row_dend = FALSE, heatmap_legend_param = list(title = "H3K27me3"))
pdf("./ALL_H3K27me3_peaks_qa-suz12_v_CAF-1_paper.pdf", height = 9)
# Draw the heatmap
draw(All_Filtered_hm)
# Close the device to save the file
dev.off()
...
# Utility function to convert rownames to a GRanges-like data.frame
parse_regions <- function(region_names) {
  region_parts <- do.call(rbind, strsplit(region_names, "\\."))
  colnames(region_parts) <- c("chr", "start", "end")
  df <- data.frame(
    chr = region_parts[,1],
...
# Merge consecutive regions (end of one == start of next)
...
# Process and write BED for each category
write_bed_from_matrix <- function(mat, rownames_subset, out_file) {
  df <- parse_regions(rownames_subset)
  df_merged <- merge_adjacent(df)
  write.table(df_merged, file = out_file, sep = "\t", quote = FALSE, col.names = FALSE, row.names = FALSE)
}

# Use the rownames from your subsets
write_bed_from_matrix(All_FilteredHeatmap_mat_map, rownames(ectopic_sort), "ectopic_regions.bed")
write_bed_from_matrix(All_FilteredHeatmap_mat_map, rownames(normal_recover_sort), "normal_recover_regions.bed")
write_bed_from_matrix(All_FilteredHeatmap_mat_map, rownames(normal_no_recover_sort), "normal_no_recover_regions.bed")
